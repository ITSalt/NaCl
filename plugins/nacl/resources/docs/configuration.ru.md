...
### `repo_checks` (опционально — команды repo-wide гейта ревью)

Repo-wide команды `lint` / `typecheck` / `test`, которые `nacl-tl-review` гоняет на wave-tip коммите. Каждый ключ опционален и используется дословно; отсутствующие ключи резолвятся автодетектом пакет-менеджера (`packageManager`, иначе lockfile). Не путать с `modules.[name].test_cmd` (внутренний цикл dev/fix). Цепочка выбирает *какие* команды бегут, но не *бегут ли*: отсутствующий скрипт всё так же валит стадию, красные проверки всё так же отказывают в VERIFIED. Полная семантика: `nacl-tl-core/references/config-schema.md` § `repo_checks`.
...
второй передаётся внешнему провайдеру из `NACL_SERVER_ROUTE_SECRET_PROVIDER`. В Codex Skills-only
пути проектный `.codex/config.toml` хранит только команду launcher и несекретные MCP-метаданные;
launcher разрешает точный источник секрета в runtime. Корневой `.mcp.json` хранит лишь непрозрачную
ссылку в Claude или явно документированных full-plugin compatibility-сценариях; это не файл
подхвата проекта в Codex. Если env или провайдер недоступен, инициализация и подключение завершаются
fail closed без demo/default fallback.
...
Пошаговые операции описаны в runbook’ах
[развёртывания на VPS](runbooks/provision-shared-graph-vps.ru.md) и
[подключения к удалённому графу](runbooks/connect-to-existing-remote-project.ru.md).
...
| Repo-wide проверки (гейт ревью) | `repo_checks.<stage>` > автодетект пакет-менеджера (`packageManager` / lockfile) > `unrunnable` |
...
## Codex Skills-only и проектный MCP

### Публичный Skills-only путь

Официальный продукт Codex — одна UI-установка Skills-only. Он читает тот же
проектный `config.yaml`; второй схемы проекта NaCl нет. `nacl-init` сначала
показывает точный план, а после подтверждения создаёт или подключает Neo4j
Community проекта, устанавливает запиненный `neo4j-mcp` и объединяет одну
безопасную запись `mcp_servers` в проектном `.codex/config.toml`. Другие MCP
entries сохраняются, а текущая задача останавливается после записи. Для MCP
handshake, схемы, чтения, подтверждённой записи и read-back нужна новая задача
в том же проекте.

### Граница совместимости и авторизации

Git/full-plugin пакет и корневой `.mcp.json` остаются каналами разработки,
Claude и явно документированной совместимости; после официальной Skills-only
установки они не нужны. Граница авторизации — Neo4j-сервер: доступ к нему
означает доступ ко всем базам проектов на нём, а другой сервер требует
отдельного grant. `graph.project_scope` — маршрутизация/provenance, а не
авторизация.

При локальном bootstrap сгенерированный пароль хранится только в gitignored
файле проекта `graph-infra/.env` с доступом владельца. Проектный launcher
проверяет этот файл и передаёт пароль дочернему процессу через его окружение.
`.codex/config.toml`, логи, аргументы инструментов, загруженный bundle и
коммиченная конфигурация не содержат сырой секрет. Официальный путь не требует
публичного MCP endpoint, hosted gateway или managed graph service.
