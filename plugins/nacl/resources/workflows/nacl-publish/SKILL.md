...
Use its read preflight and protected `ReleaseEnvironment` sequence only.
Task/domain reconciliation requires the corresponding packaged named reads;
while they are absent, graph-derived publishing is
`BLOCKED/PUBLISH_NAMED_QUERY_UNAVAILABLE`. External publication always retains
its separate explicit user confirmation.
...
`<project-root>/.tl/reconciliation/_template.json`.
