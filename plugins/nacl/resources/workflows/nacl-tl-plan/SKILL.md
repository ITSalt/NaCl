...
1. **Discover.** Use only a packaged named external-contract closure read for
   every UC in scope. The current pilot does not expose that read, so graph-
   backed discovery returns `Status: BLOCKED` with code
   `TL_NAMED_QUERY_UNAVAILABLE`; do not execute an inline graph statement. A
   user-provided confirmed file-only contract list may support planning, but it
   cannot be reported as graph-complete.

2. **Existence check.** For each confirmed contract row, the file at
...
- Re-planning is idempotent and incremental by default (full overwrite only on
  ...
  changed set from **two drift-confirmed signals only**: (1) version drift —
  `t.planned_from_version IS NOT NULL AND coalesce(uc.spec_version,0) > t.planned_from_version`;
  (2) explicit stale stamp — `review_status='stale'` on a UC or its Tasks. Do NOT
