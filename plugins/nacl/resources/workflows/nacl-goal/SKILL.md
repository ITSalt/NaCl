...
Run its `read-preflight` sequence once for graph-aware goals, then delegate any
mutation to the exact BA, SA, TL, migration, or release sequence of the chosen
leaf. Preserve project identity, principal/session/worker scope, idempotency,
lease/fence/revision, child status/code, and the goal proof/refusal gates.
...
| Alias | Packaged check reference (preview only) |
|---|---|
| `wave:<N>` | `wave` |
| `fix:<BUG-NNN>` | `fix` |
| `validate:module:<MOD-ID>` | `validate` |
| `reopened-drain` | `reopened-drain` |
...
The `intake` alias and its packaged check definition (2.10.1+) are
...
The `conduct` alias and its packaged check definition (2.18.0+) are the
...
Resolve check scripts only from the packaged `../../nacl-goal/checks/`
directory. If a declared script is absent from the installed cache, fail
closed instead of resolving a source checkout.
...
- Resolve built-in aliases from `../../nacl-goal/aliases.md`.
- Read packaged `/goal` compatibility docs and check scripts.
- Produce preview text without starting a goal loop.
- Run packaged check scripts locally when the shell is available.
...
- Use `BLOCKED` for missing package resources, missing scripts, unavailable `/goal`,
