...
- Remote mode (`config.yaml` `graph.mode: remote`, shared graph): read the
  prior verification status from the authoritative graph `Task`, never the
  per-clone `.tl/status.json`. Carry the identity and fence obtained through
  `nacl_graph_derive_worker_identity` and `nacl_graph_claim_resource`, heartbeat
  long shipping work with `nacl_graph_heartbeat_resource`, then after a
  successful push call `nacl_graph_release_resource` with a stable idempotency
  key and `APPROVE_TL_WRITE`, or use an explicitly confirmed
  `nacl_graph_handoff_resource`. Preserve a failed release as a closed result;
  do not run a package-local script. Load
