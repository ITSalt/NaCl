...
Allocate the output path with `baseline_file=$(mktemp -t 'UC###-be-baseline.XXXXXX')` and store the output in `$baseline_file`. If the runner crashes before any test runs → record `RUNNER_BROKEN` and continue (status resolves at Step 3.5).
