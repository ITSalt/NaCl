---
name: graph_tl_intake
description: |
  Graph-aware request triage: queries Neo4j to disambiguate features vs bugs.
  Routes features to graph_sa_feature, bugs to tl-fix, tasks to tl-dev.
  Graph-first equivalent of tl-intake.
  Use when: triage with graph context, batch of changes with graph, or the user says "/graph_tl_intake".
---

# /graph_tl_intake -- Graph-Aware Request Triage & Decomposition
...
**Key advantage over tl-intake:** Classification queries the graph for existing Use Cases. If a UC already exists and is detailed, the request is likely a BUG (existing behavior is broken). If no matching UC exists, it is a FEATURE (new behavior).
...
```
/graph_tl_intake "I need these changes:
...
```
/graph_tl_intake "The share button doesn't work on mobile, also I want to add
...
If Neo4j connection fails, use the same heuristic rules as tl-intake:
...
Is it new functionality or enhancement to existing behavior?
  -> YES: FEATURE (route to /graph_sa_feature)
...
From your 5 requests, I identified:
  ...
  | -> /graph_sa_feature (after Feature 1)       |
  +----------------------------------------------+
...
Execution plan:
  1. /tl-fix "Share button broken on mobile"
  2. /graph_sa_feature "Generation Controls: ..."
  3. /graph_sa_feature "Image Editing: ..."
...
**Goal:** Execute the confirmed plan -- features via `/graph_sa_feature`, bugs via `/tl-fix`, tasks via `/tl-dev`.

**Critical routing difference from tl-intake:**

| Item type | tl-intake routes to | graph_tl_intake routes to |
|-----------|---------------------|---------------------------|
| Feature | `/sa-feature` | `/graph_sa_feature` |
| Bug | `/tl-fix` | `/tl-fix` (unchanged) |
| Task | `/tl-dev` | `/tl-dev` (unchanged) |
...
```
Wave 1 (parallel):
  +-- /tl-fix "Share button broken on mobile"  <-- bug, independent
  +-- /graph_sa_feature "Generation Controls" (#1-4)  <-- independent feature

Wave 2 (after Wave 1):
  +-- /graph_sa_feature "Image Editing" (#5)  <-- depends on Feature 1 tabs UI
```
...
If all atoms are bugs -> skip graph_sa_feature, execute all via /tl-fix.

### Single item

If the user provides only one request:
- If it's a feature -> redirect to `/graph_sa_feature` directly (no decomposition needed)
...
- Then proceed with feature (/graph_sa_feature)
...
3. Route features to `/graph_sa_feature` anyway (it has its own fallback handling)
...
```
/graph_tl_intake (this skill)
  |-- Bugs     -> /tl-fix "description"
  |-- Tasks    -> /tl-dev TECH-NNN
  +-- Features -> /graph_sa_feature "description"
...
# - /graph_sa_feature creates .tl/feature-requests/FR-NNN.md and graph nodes
# - /tl-fix creates fixes and updates docs
# - /tl-dev creates TECH task implementations
```
...
- `graph_sa_feature/SKILL.md` -- graph-based feature specification workflow
