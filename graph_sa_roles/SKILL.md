...
description: |
  ...
  Use when: define system roles in graph, import BA roles, create permission matrix, graph_sa_roles.
---

# /graph_sa_roles --- System Roles & Permissions (Graph)
...
**When:** After `graph_sa_architect` and `graph_sa_domain`, when building the full role model from scratch.
...
If no DomainEntities found -- suggest running `graph_sa_domain` first.
...
Next steps:
1. `/graph_sa_uc` -- detail Use Cases with activity steps and forms
2. `/graph_sa_stories` -- create User Stories per module
```

---

## Checklist /graph_sa_roles
