---
name: graph_tl_status
description: |
  Graph-aware project status with SA coverage metrics.
  Reads Task/Wave nodes from Neo4j, falls back to status.json.
  Graph-first equivalent of tl-status.
  Use when: project status with graph, check progress with SA coverage, or the user says "/graph_tl_status".
---

# /graph_tl_status -- Graph-Aware Project Status

## Purpose

Graph-powered replacement for `/tl-status`. Computes progress from Task and Wave
...
**Critical difference from tl-status:**

| Aspect | tl-status | graph_tl_status |
|--------|-----------|-----------------|
| Data source | `.tl/status.json` | Neo4j Task/Wave nodes (primary) |
| Fallback | None | `.tl/status.json` (identical to tl-status) |
...
```
/graph_tl_status [options]
```

### Filtering Options

```
/graph_tl_status                # Full report (all sections)
/graph_tl_status --waves        # Show wave-by-wave progress only
/graph_tl_status --be           # Show only BE progress (BE dev + BE review phases)
/graph_tl_status --fe           # Show only FE progress (FE dev + FE review phases)
/graph_tl_status --tech         # Show only TECH tasks section
/graph_tl_status --stubs        # Show detailed stubs report (by severity, marker, task)
/graph_tl_status --qa           # Show QA results per task (pass/fail/pending)
/graph_tl_status --blocked      # Show blockers with dependency chains
/graph_tl_status --compact      # One-line compact summary
```

Filter behavior is identical to `/tl-status` (see filter section below).
...
Then execute exactly the same logic as `/tl-status` (read `.tl/status.json`, stub-registry, QA reports). Skip the SA Coverage section.
...
These files supplement the graph data. Read them the same way as tl-status.
...
Stubs line: read from `.tl/stub-registry.json` per-task data (same as tl-status).
...
Read from `.tl/stub-registry.json` (same logic as tl-status).
...
Check for delivery state files (same logic as tl-status):
...
Additionally, apply phase-level blocking rules (same as tl-status):
...
SA specification readiness metrics that are invisible to `/tl-status`.
...
2. Fall back to `.tl/status.json` and proceed identically to `/tl-status`.
...
3. Suggest: `Run /graph_tl_plan to create Task/Wave nodes in the graph.`

### Fallback: .tl/status.json

When in fallback mode, all logic is identical to `/tl-status`:
...
`Note: stub-registry.json not found. Run /tl-stubs to scan.`

### Inconsistent data

If phase states conflict (e.g., `phase_be: done` but `phase_review_be: pending` skipped), report the inconsistency and suggest `/tl-plan --refresh`.

### Stale data

If the most recent `Task.updated` timestamp is older than 3 days:
`Warning: Status data is stale (last updated: YYYY-MM-DD). Run /tl-status --refresh.`
...
- `/graph_sa_uc UC###` -- Detail a UC in the graph (if SA coverage gaps found)
- `/graph_tl_plan` -- Regenerate plan from graph (if tasks are missing)
