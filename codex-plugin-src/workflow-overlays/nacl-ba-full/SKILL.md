...
and use only its `ba-resource` sequence. The pilot supports protected `Board`
metadata, not the full BA domain graph. Any phase requiring an unavailable BA
label, relationship, or named read returns
`BLOCKED/BA_DOMAIN_RESOURCE_UNAVAILABLE`; never claim the full BA graph is
verified from file-only output.
