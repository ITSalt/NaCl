...
[`the packaged fix proof method`](../../nacl-goal/checks/fix.sh) as an internal
reference only; it is not a project-cwd command. Preserve non-hotfix scope:
...
3. If `config.yaml` has a graph section, attempt graph-enhanced UC discovery
   before file grep, using only a packaged named UC search. The current pilot
   does not expose that read, so report `Status: BLOCKED` with code
   `TL_NAMED_QUERY_UNAVAILABLE` for graph triage; do not execute an inline graph
   statement. File search may still produce a bounded diagnostic, but is not
   graph-authoritative.
