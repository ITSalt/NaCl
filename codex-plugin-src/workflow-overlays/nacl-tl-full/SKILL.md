...
and use only its `tl-task` sequence. Allocate or claim a `Task`, retain and
heartbeat its lease/fence, mutate with revision CAS and
`APPROVE_TL_WRITE`, read back, then release or explicitly hand off. Never
advance `done` or `verified-pending` without parseable evidence in that same
mutation.
...
alias. Load the internal workflow `../nacl-goal/SKILL.md` and
...
[`the packaged wave proof method`](../../nacl-goal/checks/wave.sh) as an internal
reference; do not expose it as a project-cwd shell command. GOAL_PROOF is
transcript evidence for the evaluator, not a replacement for local
verification. Use the closed Codex status vocabulary when the wrapper cannot
run.
...
- When writing a terminal `Task.status` to the graph, also write
  ...
  (`test-GREEN:<path>`), or use `test-UNVERIFIED` per the taxonomy. A `no-test`
  override requires the separate exact mutate input
  `evidence_confirmation: CONFIRM_NO_TEST_EVIDENCE`; absent or wrong
  confirmation is `BLOCKED` before graph access. A terminal `done` write
  without parseable evidence is a writer contract violation; report
  `Status: BLOCKED` rather than write empty.
...
working a task, call `nacl_graph_derive_worker_identity`, then
`nacl_graph_claim_resource` for the exact `Task` with a stable idempotency key,
bounded TTL, and `APPROVE_TL_WRITE`. Retain the returned fence, heartbeat long
work with `nacl_graph_heartbeat_resource`, mutate only with that fence and
revision CAS, and finish with `nacl_graph_release_resource` or an explicitly
confirmed `nacl_graph_handoff_resource`. Stamp returned identity provenance on
phase writes. Load
[`the packaged remote coordination reference`](../../nacl-tl-core/references/remote-mode-coordination.md);
never shell to a package path or infer one from the project cwd.
