...
description: |
  ...
  create board, visualize graph, or the user says "/graph_render".
---

# graph_render -- Graph to Markdown / Excalidraw Renderer
...
```
/graph_render md <command> [args] [--output <path>]
/graph_render excalidraw <command> [args]
```

## Dependencies

- `graph_core/SKILL.md` -- shared Neo4j connection and schema references
...
These factories are used by all commands below. They follow the templates and colors from `graph_core/SKILL.md`.
...
#### Color Reference (from `graph_core/SKILL.md`)
...
### `/graph_render excalidraw domain-model`
...
### `/graph_render excalidraw context-map`
...
### `/graph_render excalidraw activity <UC-ID>`
...
### `/graph_render excalidraw ba-process <BP-ID>`
...
```
/graph_render md entity DE-Order
```
...
```
/graph_render md uc UC-101 --output docs/13-usecases/UC101-order-creation.md
```

### Example: Full traceability matrix

```
/graph_render md traceability --output docs/20-traceability/ba-sa-matrix.md
```
