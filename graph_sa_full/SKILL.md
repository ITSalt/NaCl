...
description: |
  ...
  Chains all graph_sa_* skills with user confirmation gates.
  Graph-first equivalent of sa-full.
  Use when: create complete SA spec with graph, or the user says "/graph_sa_full".
---

# /graph_sa_full --- Full SA Specification (Graph Orchestrator)
...
This is the graph-first equivalent of `/sa-full`. The orchestration pattern is identical; only the storage backend changes (Neo4j graph instead of docs/ files).

---

## Architecture

graph_sa_full is an **orchestrator** that manages invocation of specialized graph SA agents in the correct order. It does NOT execute phases itself --- it delegates each phase to a dedicated skill via a Task agent (sub-agent with isolated context).
...
2. L0 launches: `Launch Task agent: /graph_sa_[skill] [mode]`
...
| graph_sa_    |    | graph_sa_    |    | graph_publish|
...
### Phase 1: Architecture -> `/graph_sa_architect full`

**Launch:** `Launch Task agent: /graph_sa_architect full`
...
### Phase 2: Domain Model -> `/graph_sa_domain` (per module)

**Launch:** For each module --- a separate Task agent (sequentially):

```
Launch Task agent: /graph_sa_domain {module_id}
```
...
### Phase 3: Roles -> `/graph_sa_roles full`

**Launch:** `Launch Task agent: /graph_sa_roles full`
...
### Phase 4: UC Stories -> `/graph_sa_uc stories`

**Launch:** `Launch Task agent: /graph_sa_uc stories`
...
### Phase 6: UI -> `/graph_sa_ui full`

**Launch:** `Launch Task agent: /graph_sa_ui full`
...
### Phase 7: Validation -> `/graph_sa_validate full`

**Launch:** `Launch Task agent: /graph_sa_validate full`
...
### Phase 8: Finalize -> `/graph_sa_finalize full`

**Launch:** `Launch Task agent: /graph_sa_finalize full`
...
### Phase 9: Publish (optional) -> `/graph_publish docmost`
...
### Phase 10: Handoff to TL (optional) -> `/graph_tl_plan`
...
2. Show summary to user:
   ...
   Options:
   1. /graph_tl_plan  --- create development plan (tasks, waves, api-contracts)
   2. Stop            --- specification complete, development later
   ```

3. On option 1: `Launch Task agent: /graph_tl_plan`
...
```
=== graph_sa_full Progress ===

[########------------] 40%

- [x] Phase 1:  Architect (graph_sa_architect)  --- {N} modules, Context Map
- [x] Phase 2:  Domain (graph_sa_domain)         --- {N} entities, {M} attributes
- [x] Phase 3:  Roles (graph_sa_roles)            --- {P} roles, permissions matrix
- [x] Phase 4:  UC Stories (graph_sa_uc stories)  --- {Q} UCs registered
- [ ] Phase 5:  UC Detail (graph_sa_uc detail)     <- next
- [ ] Phase 6:  UI (graph_sa_ui)
- [ ] Phase 7:  Validate (graph_sa_validate)
- [ ] Phase 8:  Finalize (graph_sa_finalize)
- [ ] Phase 9:  Publish (graph_publish)            (optional)
- [ ] Phase 10: TL Handoff (graph_tl_plan)         (optional)
```
...
5. Return to open questions in Phase 7 (graph_sa_validate) --- validation will catch gaps
...
2. /graph_tl_plan          --- create development plan from graph
...
calls:
  - Phase 1:  graph_sa_architect  (Task agent)
  - Phase 2:  graph_sa_domain     (Task agent, per module)
  - Phase 3:  graph_sa_roles      (Task agent)
  - Phase 4:  graph_sa_uc stories (Task agent)
  - Phase 5:  graph_sa_uc detail  (Task agent, per Primary UC)
  - Phase 6:  graph_sa_ui         (Task agent)
  - Phase 7:  graph_sa_validate   (Task agent)
  - Phase 8:  graph_sa_finalize   (Task agent)
  - Phase 9:  graph_publish       (Task agent, optional)
  - Phase 10: graph_tl_plan       (Task agent, optional)
```

---

## Checklist /graph_sa_full
...
### Orchestration
- [ ] Phase 1: graph_sa_architect completed and confirmed
- [ ] Phase 2: graph_sa_domain completed for all modules
- [ ] Phase 3: graph_sa_roles completed and confirmed
- [ ] Phase 4: graph_sa_uc stories completed and confirmed
- [ ] Phase 5: graph_sa_uc detail completed for all Primary UCs (minimum)
- [ ] Phase 6: graph_sa_ui completed and confirmed
- [ ] Phase 7: graph_sa_validate passed (0 critical errors)
- [ ] Phase 8: graph_sa_finalize completed
- [ ] Phase 9: graph_publish executed (if user confirmed)
- [ ] Phase 10: graph_tl_plan proposed (if user ready for development)
