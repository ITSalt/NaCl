---
name: graph_ba_validate
description: |
  Validate BA model consistency via Cypher: L1-L8 internal checks,
  XL1-XL5 cross-validation with SA layer. Read-only, no writes.
  Graph-first equivalent of ba-validate.
  Use when: validate BA model, check consistency, or the user says "/graph_ba_validate".
---

# /graph_ba_validate -- BA Model Validation (Graph)
...
2. Suggest the user runs `/graph_ba_import_doc`, `/graph_ba_from_board`, or `/graph_ba_process` first.
3. Explain that `/graph_ba_validate` works only with a populated Neo4j graph.
...
- `scope=cross` --> STOP, report that SA layer is not populated. Suggest `/graph_sa_domain` or `/graph_sa_architect` first.
