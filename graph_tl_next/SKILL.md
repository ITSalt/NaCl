...
description: |
  ...
  Reads Task/Wave from Neo4j, enriches with UC entity/form names.
  Graph-first equivalent of tl-next.
  Use when: next task with graph, what to work on, or the user says "/graph_tl_next".
---

# /graph_tl_next -- Graph-Aware Next Task Recommendation

## Purpose

Graph-powered replacement for `/tl-next`. Queries Task and Wave nodes from Neo4j
...
**Critical difference from tl-next:**

| Aspect | tl-next | graph_tl_next |
|--------|---------|---------------|
...
```
/graph_tl_next --be        # Only BE development tasks
/graph_tl_next --fe        # Only FE development tasks
/graph_tl_next --tech      # Only TECH tasks
/graph_tl_next --review    # Only review tasks (BE or FE)
/graph_tl_next --sync      # Only sync verification tasks
/graph_tl_next --qa        # Only QA testing tasks
/graph_tl_next --wave N    # Only tasks from wave N
/graph_tl_next --list      # Show top 5 candidates with scores
```
...
Try Neo4j query (tl_active_wave)
  ...
  +-- Connection error OR empty results -> FALLBACK MODE
      ...
      |     +-- Yes -> file-based recommendation (same as tl-next)
...
-- ------ ------------------------- ------------ --------------- ----------------------
1  135    UC003-BE Delete Order     BE Dev       Order,AuditLog  /tl-dev-be UC003
2  125    UC001-FE Create Order     FE Dev       Order,Customer  /tl-dev-fe UC001
3  110    UC002-FE Edit Order       FE Dev       Order,OrderItem /tl-dev-fe UC002
4   85    TECH-003 Error Handling   TECH Dev     --              /tl-dev TECH003
5   60    UC004-BE List Orders      BE Dev       Order           /tl-dev-be UC004
...
When Neo4j is unavailable, operate identically to `tl-next`:
...
| No TL nodes in graph | Graph | `No Task/Wave nodes in graph` | `/graph_tl_plan` |
| No `.tl/` directory | Fallback | `Project not initialized for TeamLead workflow` | `/tl-plan` or `/graph_tl_plan` |
| Missing `master-plan.md` | Fallback | `Wave info unavailable, priority-only mode` | `/tl-plan` |
| Corrupted `status.json` | Fallback | `Warning: status.json is invalid` | `/tl-plan --refresh` |
| Empty task list | Both | `No tasks found` | `/tl-plan` or `/graph_tl_plan` |
...
- `/graph_tl_next --list` -- View top 5 candidates with scores
