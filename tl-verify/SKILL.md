...
description: |
  ...
  Runs /tl-verify-code first (cheap, catches 60%+ issues), then /tl-qa if needed.
...
|---------|------|-------|---------|
| **Pre-ship QA** (tl-full Step 7) | During development | Local dev servers | Catches code bugs cheaply, before any git/CI overhead |
...
**Precondition:** Code must be committed and pushed. If not yet pushed → run `/tl-deliver` or `/tl-ship` first.
...
```
/tl-verify UC028                    # verify specific UC
/tl-verify --task ELE-644           # verify by YouGile task code
/tl-verify --all                    # verify all ReadyToTest tasks from YouGile
```
...
| Test credentials | `config.yaml → credentials` (passed to tl-qa for E2E tests) |
...
- Run `/tl-verify-code` (via Skill tool or inline)
...
- Run `/tl-qa` — browser-based E2E test
- **REQUIRE from tl-qa:** screenshots at every step + HTML report
...
- `code-analysis.md` — /tl-verify-code result (YAML block)

**HTML report structure:**
Same as tl-qa Step 5b format: standalone HTML with inline CSS, summary table, test steps, screenshot grid, verdict badge.
...
- **FAIL** -- move to Reopened
  ...
  - The developer can then use `/tl-fix` to address the issues
...
- `tl-verify-code/SKILL.md` -- code analysis sub-skill
- `tl-qa/SKILL.md` -- E2E testing sub-skill
