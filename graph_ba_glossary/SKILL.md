---
name: graph_ba_glossary
description: |
  Build ubiquitous language glossary in Neo4j: terms, definitions, synonyms, links.
  Graph-first equivalent of ba-glossary.
  Use when: create glossary with graph, define terms, or the user says "/graph_ba_glossary".
---

# /graph_ba_glossary --- Glossary in Neo4j
...
# Shared references:
- graph_core/SKILL.md                    # ID format, schema, conventions
```
...
When running `/graph_ba_glossary incremental`:
...
**Next steps:**
1. /graph_ba_analyze --- validate the board against the graph
2. /ba-validate --- check glossary coverage (level L7)
3. /ba-handoff --- hand off glossary to SA phase (sa_term mapping)
```
...
> No BA nodes found in the graph (no BusinessEntity, BusinessRole, BusinessProcess, WorkflowStep, or BusinessRule). Populate the graph first with `/graph_ba_process`, `/graph_ba_context`, or `/graph_ba_from_board`.
