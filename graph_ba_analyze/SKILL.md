...
description: |
  ...
  найти проблемы на доске, board analysis, graph_ba_analyze.
---

# /graph_ba_analyze --- Анализ Excalidraw-доски
...
> No Excalidraw boards found in `{$boards_dir}/`. Create a board first with `/graph_ba_import` or `/graph_ba_draft`, then run `/graph_ba_analyze`.
...
> No synced elements found on the board. Run `/graph_ba_sync` to synchronize board elements with the Neo4j graph.
...
> No synced elements on the board. Run `/graph_ba_sync` to synchronize.
...
5. **Synchronize with graph** --- {N} elements are ready for sync. Run `/graph_ba_sync` to push to Neo4j.
6. **Fix graph mismatches** --- {N} elements have name mismatches between board and graph. Update the board or run `/graph_ba_sync --force` to overwrite.
...
# Shared references:
- graph_core/SKILL.md                                 # Excalidraw format, customData structure
