...
- **`${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md`** --- Excalidraw JSON format, element types, color coding, customData structure, layout guidelines, ID generation rules.
...
Every visual element consists of a **shape** (rectangle/diamond) plus a **bound text** element. Reference `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md` for the canonical template.
...
For each step, create a rectangle with bound text. Apply colors from `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md`:
...
Do not invent colors. Use only the backgroundColor and strokeColor values defined in `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md`:
...
| `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md` | Read | Excalidraw format, customData, colors, layout rules |
...
- [ ] Colors match `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md` exactly
