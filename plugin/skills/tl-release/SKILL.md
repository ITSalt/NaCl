...
**emergency mode** — see
`${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md`. Emergency mode is a
...
loudly recorded — see `${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md`).
...
See `${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/config-schema.md` § "W4 PR/CI
Carve-Out (binding)".
...
Full schema and rules: `${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md`.
...
`${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/classify-pr-merge.mjs` (pure, never opens Neo4j, pinned by
`classify-pr-merge.test.mjs`); this skill gathers the graph rows + trailers and feeds them in,
so the verdict is reproducible:
```bash
node ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/classify-pr-merge.mjs '<pr-json>'   # or a JSON array of PRs
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/wait-for-ci.sh watch --branch {main_branch} --since "$merge_iso" \
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/health-check.sh --url "{production_url}{health_endpoint}"
...
Resolution options:
  ...
  [4] Invoke emergency mode (NACL_EMERGENCY=1 +
      ...
      VERIFIED — see ${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md.
```
...
| `NACL_EMERGENCY=1` + companion env vars set | Emergency mode: every Strict-Only gate refusal prints a banner, the run advances, the terminal Status: carries `(emergency-bypass)` suffix (NEVER VERIFIED), event recorded in `.tl/emergencies/`. See `${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md`. |
