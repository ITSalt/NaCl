...
1. Determine branch name (slugify the title via the single-authority formatter — same
   lowercase/hyphens/≤50 rule as `/nacl:tl-ship`, pinned by `${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.test.sh`):
   - If `--branch`: use as-is
   - If `--feature FR-001`: `feature/FR-001-$(bash ${CLAUDE_PLUGIN_ROOT}/nacl-core/scripts/branch.sh slug "<title>")`
...
warning (see `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md` § Task.verification_evidence).
...
`${CLAUDE_PLUGIN_ROOT}/nacl-tl-core/references/emergency-mode.md`. Emergency mode does
...
3b. **Evidence-completeness gate** — verify every `done` / `verified-pending`
   ...
   (see `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md` § Task.verification_evidence).
   ...
   - If the query returns **any rows**: HALT immediately.
     ...
     `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md`. Resolution options:
...
Development:
  ...
  (taxonomy: `${CLAUDE_PLUGIN_ROOT}/nacl-core/SKILL.md`). The same string is what
