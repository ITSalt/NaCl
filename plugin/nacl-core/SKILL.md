...
Liveness probe and start/fix tool: `$(nacl-home)/nacl-core/scripts/graph-doctor.mjs`
(`node "$(nacl-home)/nacl-core/scripts/graph-doctor.mjs" [--fix]`) —
...
`$(nacl-home)/nacl-core/scripts/graph-doctor.mjs`, pinned by its test.
...
> Tell me "start the graph" and I will run `node "$(nacl-home)/nacl-core/scripts/graph-doctor.mjs" --fix` via Bash (works in Claude Code Desktop and CLI).
...
Neo4j is not reachable at bolt://localhost:{$neo4j_bolt_port}. Tell me "start the graph" (I will run node "$(nacl-home)/nacl-core/scripts/graph-doctor.mjs" --fix) or start it from the project root: docker compose -f graph-infra/docker-compose.yml up -d (local) / ~/.nacl/sidecar/<project_scope>.sh (remote).
...
- TL task claiming in remote mode is protected by the claim-lock
  (`$(nacl-home)/nacl-core/scripts/claim-task.mjs`).
...
> **Behavior Slice (`SLC-{NNN}-{PascalName}`)** is the graph-native acceptance
> scenario (label `:Slice`), written by `nacl-sa-uc slices`. `{NNN}` is the
> parent UC number (`UC-006` → `006`) — the infix makes repeated scenario names
> (every UC has a `HappyPath`) unique and enables scoped filtering by
> `STARTS WITH 'SLC-NNN-'`. `{PascalName}` derives from a latin short scenario
...
— a true left-pad that never truncates. The JS authority is `$(nacl-home)/nacl-core/scripts/nacl-ids.mjs`
(`node $(nacl-home)/nacl-core/scripts/nacl-ids.mjs <kind> <next_int> [parentId]`), pinned by
`$(nacl-home)/nacl-core/scripts/nacl-ids.test.mjs`; use it when formatting an id OUTSIDE Cypher. In-Cypher,
