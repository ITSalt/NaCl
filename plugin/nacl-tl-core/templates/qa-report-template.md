...
`nacl-tl-full` skill, `nacl-tl-status` skill
...
- **{{AC_ID}}**: {{Criterion}} -- {{Reason, e.g., "Verified in unit tests during nacl-tl-review"}}
...
{{For PASS: "Task is ready for completion. Proceed to nacl-tl-full."}}
{{For FAIL: "Task requires rework. Fix the listed bugs and failed criteria, then request re-test via nacl-tl-qa."}}
...
|---------|---------|-------------|
| `PASS` | All UI criteria met, no critical/major bugs | Proceed to nacl-tl-full |
