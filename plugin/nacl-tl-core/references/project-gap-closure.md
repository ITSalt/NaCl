...
1. `$(nacl-home)/nacl-tl-core/references/strict-mode-changes.md` — what the eleven
...
2. `$(nacl-home)/nacl-tl-core/references/gate-fire-catalog.md` — the eleven canonical
   gate-fire points (G1..G11) with SKILL.md file:line citations and
   canonical fixture paths.
3. `$(nacl-home)/nacl-tl-core/references/config-schema.md` — `project_kind`,
   `runtime_assets`, `build.*`, `deploy.smoke.endpoints` — the
   config-side surface every gate keys off.
4. `$(nacl-home)/nacl-tl-core/references/emergency-mode.md` — the only sanctioned
...
- `$(nacl-home)/nacl-tl-core/references/strict-mode-changes.md` — what the eleven
  skills now do.
- `$(nacl-home)/nacl-tl-core/references/gate-fire-catalog.md` — the eleven canonical
  gate-fire points (G1..G11), each citing SKILL.md file:line and W11
  pilot section.
- `$(nacl-home)/nacl-tl-core/references/config-schema.md` — `project_kind`,
  `runtime_assets`, `build.*`, `deploy.smoke.endpoints`.
- `$(nacl-home)/nacl-tl-core/references/emergency-mode.md` — the only sanctioned
  bulk-bypass surface.
