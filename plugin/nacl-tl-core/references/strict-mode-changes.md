...
`$(nacl-home)/nacl-tl-core/references/emergency-mode.md`). Strict is the single,
...
`$(nacl-home)/nacl-tl-core/references/config-schema.md:61-95`)
...
`$(nacl-home)/nacl-tl-core/references/config-schema.md:126-309` —
...
lives at `$(nacl-home)/nacl-tl-core/references/emergency-mode.md`. This section names
...
(`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:23-45`)
...
(`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:60-123`)
(provenance: `~/.nacl/wave-evidence/W4-blocking-release.md:28`).
...
(`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:151-156`). The terminal
...
- Gate-fire catalog: `$(nacl-home)/nacl-tl-core/references/gate-fire-catalog.md` — the
  eleven canonical fire points, each citing SKILL.md file:line and
  W11-pilot section.
- Project GAP closure runbook:
  `$(nacl-home)/nacl-tl-core/references/project-gap-closure.md` — for per-project GAP
  planners inspecting an existing project against strict mode.
- Config schema: `$(nacl-home)/nacl-tl-core/references/config-schema.md` —
  `project_kind`, `runtime_assets`, `build.*`, `deploy.smoke.endpoints`.
- Emergency mode: `$(nacl-home)/nacl-tl-core/references/emergency-mode.md` — full
