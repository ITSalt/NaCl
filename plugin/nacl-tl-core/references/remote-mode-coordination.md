...
tool `$(nacl-home)/nacl-core/scripts/claim-task.mjs` (do not hand-write the Cypher — run the tool's query via
...
```
claim:   node $(nacl-home)/nacl-core/scripts/claim-task.mjs claim   --task <id> --dev <NACL_DEVELOPER_ID> --json
release: node $(nacl-home)/nacl-core/scripts/claim-task.mjs release --task <id> --dev <NACL_DEVELOPER_ID> --json
```
...
`NACL_DEVELOPER_ID="$(node $(nacl-home)/nacl-core/scripts/resolve-developer-id.mjs --project-root .)"`.
