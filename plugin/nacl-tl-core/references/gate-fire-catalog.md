...
(`$(nacl-home)/nacl-tl-core/references/project-gap-closure.md`). Any agent inspecting a
...
**Owning artifact:** `$(nacl-home)/nacl-tl-core/references/emergency-mode.md`. Hosted by
...
(`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:175-191`).
...
`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:80-83`:
...
And `$(nacl-home)/nacl-tl-core/references/emergency-mode.md:151-156`:
...
1. Bypass banner on stderr, one per gate
   (`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:66-78`).
...
3. `"emergency"` key appended to `.tl/release-status.json` (for release /
   deliver / deploy / ship; `$(nacl-home)/nacl-tl-core/references/emergency-mode.md:93-105`).
4. Blockquote line appended to `.tl/changelog.md` under the in-flight
   version heading (`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:107-116`).
5. `postmortem_feed.tagged: true` set on the event file so the next
   postmortem includes the event (`$(nacl-home)/nacl-tl-core/references/emergency-mode.md:118-123`).
...
`$(nacl-home)/nacl-tl-core/references/project-gap-closure.md`. Agents using this catalog
