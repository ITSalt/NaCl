...
description: |
  ...
  Chains tl-ship, tl-verify, and tl-deploy into a single continuous pipeline.
  Use when: deliver to staging, push and verify, ship feature branch,
  or the user says "/tl-deliver".
---
...
```
/tl-deliver                          # deliver current branch
/tl-deliver --branch feature/FR-001  # deliver specific branch
/tl-deliver --feature FR-001         # deliver by feature request ID
/tl-deliver --skip-verify            # push + CI only, no staging verification
/tl-deliver --skip-deploy            # push + CI + verify, no health check
/tl-deliver --env staging            # target environment (default: staging)
/tl-deliver --env production         # target production (extra safety checks)
```
...
2. Check for uncommitted changes:
   ...
   ```
   - If uncommitted changes exist → **STOP**: "Uncommitted changes detected. Run /tl-conductor or commit manually first."
...
2. Create PR if feature-branch strategy and PR doesn't exist:
   ...
   - If no PR:
     ...
     ## Delivery
     Automated delivery via /tl-deliver
...
2. For each UC (or batch via `--all`):
   ...
   ```
   Execute /tl-verify UC###
   ```
   
   tl-verify internally runs:
   - `/tl-verify-code` (static analysis, fast)
   - `/tl-qa` (E2E on staging, only if code analysis says PASS_NEEDS_E2E)

3. Collect results:
   - PASS: UC verified on staging
   - FAIL: UC has issues (code analysis or E2E failure)

4. YouGile transitions (handled by tl-verify):
...
6. Decision:
   ...
   - If ALL UCs FAIL → **STOP**, recommend /tl-reopened
...
Next:
  /tl-release          — when ready for production
  /tl-reopened         — if verification issues found
═══════════════════════════════════════════════════════════════
```
...
Next:
  /tl-reopened UC029   — fix and re-verify
  /tl-release          — release verified UCs only
═══════════════════════════════════════════════════════════════
```
...
```
tl-conductor → calls /tl-deliver after all development complete
tl-deliver internally:
  Step 2: uses git + gh CLI directly (NOT /tl-ship — to avoid double test/build)
  Step 4: delegates to /tl-verify (which runs /tl-verify-code + /tl-qa)
  Step 5: uses curl for health check (NOT /tl-deploy — simpler, no pipeline monitoring needed)
```

**Why not use tl-ship?** tl-ship runs tests, build, commits, and pushes. But tl-deliver receives code that's already committed (by tl-conductor). Running tl-ship would duplicate test/build and try to commit when there's nothing to commit. tl-deliver only needs to push and create PR.

**Why not use tl-deploy?** tl-deploy is designed to monitor a CI/CD pipeline triggered by a push and then do health checks. tl-deliver already monitors CI (Step 3) and does health checks (Step 5). Using tl-deploy would add unnecessary indirection.
