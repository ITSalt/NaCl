---
name: graph_sa_feature
description: |
  Incremental feature specification via Neo4j graph. Impact analysis through
  Cypher traversal, selective SA skill invocation, FeatureRequest artifact.
  Graph-first equivalent of sa-feature.
  Use when: add feature with graph, new functionality, or the user says "/graph_sa_feature".
---

# /graph_sa_feature -- Incremental Feature Specification (Graph)

## Your Role

You are a **system analyst** who adds new features to an existing, already-specified project. Unlike `/graph_sa_architect` (which builds module decomposition from scratch), you surgically update only the affected parts of the specification by leveraging **Neo4j graph traversal** for impact analysis.
...
2. Selective execution: Run only the graph_sa_* skills that are needed
...
| Skill | When invoked |
|-------|-------------|
| `graph_sa_architect` | New module needed |
| `graph_sa_domain` | New or modified entities/enums |
| `graph_sa_roles` | New permissions or roles |
| `graph_sa_uc` | New or modified use cases |
| `graph_sa_ui` | New or modified screens |
| `graph_sa_validate` | Incremental validation (`--scope=intra-uc`) |
...
```
/graph_sa_feature "Add payment system with YooKassa integration"
/graph_sa_feature "Add VK and email auth alongside existing Telegram"
/graph_sa_feature "Admin panel for managing prompts and viewing statistics"
```
...
|               |    |  traversal)      |    |  graph_sa_*)     |    | (graph_sa_       |    | (graph indexes)  |    |           |
+---------------+    +------------------+    +------------------+    |  validate)       |    +------------------+    +-----------+
                                                                     +------------------+
```
...
This is the **key advantage** of graph_sa_feature over sa-feature: impact is detected by traversing the live graph, not by scanning markdown files.
...
|------|----------|----------------|
| Architecture | Does this need a new module/bounded context? | `graph_sa_architect module` |
| Domain: new | Does this introduce new entities or enums? | `graph_sa_domain CREATE` |
| Domain: modify | Does this change existing entities? | `graph_sa_domain MODIFY` |
| UCs: new | Does this create new user interaction flows? | `graph_sa_uc` (create) |
| UCs: modify | Does this change existing UC behavior? | `graph_sa_uc` (update) |
| Roles | Does this add new permissions or roles? | `graph_sa_roles` |
| Screens: new | Does this need new UI screens? | `graph_sa_ui` |
| Screens: modify | Does this change existing screens? | `graph_sa_ui` (update) |
...
**Goal:** Run only the flagged graph_sa_* skills, in dependency order.

Execute ONLY the steps that were flagged in Phase 2. Skip everything else.

#### 3a. Architecture (if new module flagged)

Invoke `/graph_sa_architect module [module_name]` via Skill tool:
...
If Skill tool unavailable, create Module node manually following graph_sa_architect conventions.

#### 3b. Domain Model (if new/modified entities flagged)

For each new entity:
- Invoke `/graph_sa_domain CREATE [entity_name]` via Skill tool or manually:
...
- Invoke `/graph_sa_domain MODIFY [entity_name]` via Skill tool or manually:
...
Invoke `/graph_sa_roles` via Skill tool or manually:
...
2. Invoke `/graph_sa_uc [UC_number]` via Skill tool or create manually:
...
- Invoke `/graph_sa_uc [UC_number] --mode=update` via Skill tool or edit manually:
...
Invoke `/graph_sa_ui` via Skill tool or manually:
...
Run `/graph_sa_validate` with scope limited to affected nodes. The advantage of graph validation is that scoping is precise -- only the affected subgraph is checked.
...
Invoke `/graph_sa_validate --scope=intra-uc` via Skill tool if available.
...
- Or create Glossary nodes in graph if using graph_ba_glossary
...
| Source | /graph_sa_feature "[original description]" |
...
## Skills Invoked
- [list of graph_sa_* skills that were actually invoked during this feature spec]
```
...
Next steps:
  /tl-plan --feature FR-NNN  -- create dev tasks
  /graph_sa_validate --full  -- full validation
  /graph_sa_feature "..."    -- add another feature
...
2. In Phase 3: run graph_sa_domain MODIFY for entities in different modules
...
3. If no modules exist yet, suggest running `/graph_sa_architect` first
