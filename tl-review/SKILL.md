...
description: |
  ...
  approve development, or the user says "/tl-review UC### --be" or "/tl-review UC### --fe".
...
|------|---------|-------------|-----------|--------|--------------|
| Backend | `/tl-review UC### --be` | task-be.md, test-spec.md, impl-brief.md, acceptance.md, result-be.md | BE 8-category | `review-be.md` | `phases.review_be` |
| Frontend | `/tl-review UC### --fe` | task-fe.md, test-spec-fe.md, impl-brief-fe.md, acceptance.md, result-fe.md | FE 10-category | `review-fe.md` | `phases.review_fe` |
| TECH | `/tl-review TECH###` | task.md, result.md | Standard BE | `review.md` | `status` (top-level) |
...
4. Apply classification from `tl-core/references/stub-tracking-rules.md`
...
Action: Resolve all CRITICAL stubs, re-run /tl-stubs, resubmit for review.
Run: /tl-dev-be UC### --continue  |  /tl-dev-fe UC### --continue  |  /tl-dev TECH### --continue
```
...
Write to `review-be.md` (BE), `review-fe.md` (FE), or `review.md` (TECH) using `tl-core/templates/review-template.md`. Include: summary, stub gate result, files reviewed, acceptance verification, checklist findings, issues, test results, TDD compliance, positive observations, decision, next steps.
...
Used for `--be` reviews and TECH reviews. Reference: `tl-core/references/review-checklist.md`.
...
Used for `--fe` reviews. Reference: `tl-core/references/fe-review-checklist.md`.
...
Next: /tl-dev-fe UC### (start frontend) or /tl-sync UC### (verify sync)
```
...
Next: /tl-sync UC### (verify BE<>FE sync) or /tl-qa UC### (E2E testing)
```
...
Run: /tl-dev-be UC### --continue   (BE rejections)
Run: /tl-dev-fe UC### --continue   (FE rejections)
Run: /tl-dev TECH### --continue    (TECH rejections)
```
...
- BE rejected -> `/tl-dev-be UC### --continue` (reads `review-be.md`)
- FE rejected -> `/tl-dev-fe UC### --continue` (reads `review-fe.md`)
- TECH rejected -> `/tl-dev TECH### --continue` (reads `review.md`)
...
Run the appropriate development skill first:
  /tl-dev-be UC###   (backend)
  /tl-dev-fe UC###   (frontend)
  /tl-dev TECH###    (TECH tasks)
```
...
Complete development first:
  /tl-dev-be UC###  |  /tl-dev-fe UC###  |  /tl-dev TECH###
```
...
Usage:
  ...
  /tl-review TECH###       (TECH task, no flag needed)
```
...
Run: /tl-dev-be UC###  |  /tl-dev-fe UC###  |  /tl-dev TECH###
```
...
| Task | Reference |
|------|-----------|
| BE review checklist | `tl-core/references/review-checklist.md` |
| FE review checklist | `tl-core/references/fe-review-checklist.md` |
| Stub tracking | `tl-core/references/stub-tracking-rules.md` |
| Code style (BE) | `tl-core/references/code-style.md` |
| FE code style | `tl-core/references/fe-code-style.md` |
| TDD workflow | `tl-core/references/tdd-workflow.md` |

## Templates

- `tl-core/templates/review-template.md` -- Review result template
...
**BE Approved:** `/tl-dev-fe UC###` (start FE) or `/tl-sync UC###` (verify sync)
**FE Approved:** `/tl-sync UC###` (sync check) or `/tl-qa UC###` (E2E testing)
**TECH Approved:** `/tl-docs TECH###` (documentation) or `/tl-next` (next task)
**Any Rejected:** `/tl-dev-be UC### --continue` | `/tl-dev-fe UC### --continue` | `/tl-dev TECH### --continue`
