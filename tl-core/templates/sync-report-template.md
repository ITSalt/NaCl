...
**Next Step**:
- PASS: Task proceeds to `tl-review`
- PASS_WITH_WARNINGS: Task proceeds to `tl-review` with warnings included in review checklist
- FAIL: Task returns to `tl-dev` for fixes. Sync iteration {{N}}/3.
```
...
|---------|---------|---------------|-----------|
| `PASS` | All checks passed, no issues | No | Proceed to `tl-review` |
| `PASS_WITH_WARNINGS` | No blockers, but warnings exist | No | Proceed to `tl-review` (warnings in checklist) |
| `FAIL` | At least one BLOCKER found | Yes | Return to `tl-dev` for fixes |
