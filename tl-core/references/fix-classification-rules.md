...
Reference document for the **tl-fix** skill. Defines which level a fix belongs to and what actions are required beyond the code change itself.
...
| API endpoint (contract) | `api-contract.md` + UC spec | manually in tl-fix | L2 |
| New endpoint | UC spec | `sa-uc --mode=update` | L2 |
| UC flow | `docs/14-usecases/` | `sa-uc --mode=update` | L2 |
| Screen / UI | `docs/15-interfaces/screens/` | manually in tl-fix | L2 |
