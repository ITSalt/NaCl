...
A reference document for the **tl-fix** skill. Defines which SA document needs to be updated for a specific type of code change, and which tool to use.
...
| API endpoint contract change (response code, request/response body) | `api-contract.md` + UC spec | Manual within tl-fix | L2 |
...
| Screen / UI (new elements, layout change, modals) | `docs/15-interfaces/screens/` | Manual within tl-fix | L2 |
...
**How to update:** Manually within tl-fix:
