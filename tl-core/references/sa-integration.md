...
**CRITICAL**: Task files created by `tl-plan` must be **self-sufficient**. Development agents (`tl-dev-be`, `tl-dev-fe`, `tl-dev`) should NOT read original SA artifacts during development. Everything needed for implementation must be embedded in the task files.
...
When `tl-plan` processes SA artifacts, read in this sequence:
...
A Use Case Main Flow contains both UI and API steps mixed together. `tl-plan` must split them into separate BE and FE tasks.
...
Before starting `tl-plan`, verify the SA artifacts are complete:
...
|------|----------------|------------------|---------|
| Task list + priorities | `_uc-index.md` | `_uc-index.md` | tl-plan (master-plan.md) |
| Task details | `UC*.md` | `UC*.md` | tl-plan (task-be + task-fe) |
| Entity context | `entities/*.md` | `entities/*.md` | tl-plan (task-be Context Extract) |
| Status values | `enumerations/*.md` | `enumerations/*.md` | tl-plan (task-be + api-contract) |
| Form specs | `screens/*.md` | `screens/*.md` | tl-plan (task-fe + impl-brief-fe) |
| Navigation | `15-interfaces/_navigation.md` | `15-interfaces/_navigation.md` | tl-plan (task-fe Routes) |
| Layouts | -- | `15-interfaces/layouts/` | tl-plan (impl-brief-fe Layout) |
| Components | -- | `15-interfaces/components/` | tl-plan (impl-brief-fe Components) |
| Design tokens | -- | `15-interfaces/design-tokens/` | tl-plan (impl-brief-fe Styling) |
| Functional reqs | `FR-*.md` | `FR-*.md` | tl-plan (acceptance.md) |
| Non-functional reqs | `NFR-*.md` | `NFR-*.md` | tl-plan (acceptance.md) |
