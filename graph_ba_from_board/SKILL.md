...
description: |
  ...
  Invocation: /graph_ba_from_board <command> [arguments]
---

# /graph_ba_from_board --- BA Board Orchestrator
...
- `graph_ba_import_doc/SKILL.md` --- document parsing and board generation
- `graph_ba_analyze/SKILL.md` --- board completeness and diff analysis
- `graph_ba_sync/SKILL.md` --- board-to-Neo4j synchronization

**You DO read:** `graph_core/SKILL.md` --- shared Excalidraw format, customData, colors, layout, ID rules.
...
```
/graph_ba_from_board <command> [arguments]
```
...
| `validate` | `[board_path]` | Run graph_ba_validate on the synced BA model |
| `handoff` | `[board_path]` | Run graph_ba_handoff to create BA→SA traceability |
...
2. **Check if board already exists.** If it does, report:
   > Board `{$boards_dir}/{project_name}-board.excalidraw` already exists ({N} elements). Use `/graph_ba_from_board import <file>` to add content, or `/graph_ba_from_board analyze` to review it.

   Set it as the active board and stop.

3. **Create the directory** `{$boards_dir}/` if it does not exist.

4. **Generate an empty board** with a swimlane scaffold per `graph_core/SKILL.md` templates.
   ...
   Top-level JSON: `{type: "excalidraw", version: 2, source: "graph_ba_from_board", elements: [...], appState: {viewBackgroundColor: "#ffffff", gridSize: null}, files: {}}`.
...
6. **Report:**
   ...
   **Scaffold:** 3 swimlane placeholders (edit names in Excalidraw)
   ...
   1. Import a client document: `/graph_ba_from_board import <file_path>`
   2. Or open the board in Excalidraw (http://localhost:{$excalidraw_port}) and draw manually.
   3. After editing, run `/graph_ba_from_board analyze` to check completeness.
   ```
...
```
/graph_ba_from_board import <file_path>
```
...
3. **Execute import (delegate to graph_ba_import_doc logic).**

   Run all 4 phases from `graph_ba_import_doc/SKILL.md`:
...
4. **Auto-trigger analysis (delegate to graph_ba_analyze logic).**

   Immediately after import completes, run the analysis phases from `graph_ba_analyze/SKILL.md`:
...
5. **Combined report.** Present both the import summary and the analysis report in a single output:
   ...
   **Board:** {active_board_path}
   ...
   2. After editing, run `/graph_ba_from_board analyze` to re-check.
   3. When ready, run `/graph_ba_from_board sync` to push to Neo4j.
   ```
...
```
/graph_ba_from_board analyze [board_path]
```
...
2. **Execute all 5 phases from `graph_ba_analyze/SKILL.md`:**
   - Phase 1: Read Board
...
3. **Append orchestrator recommendations** to the report:
   ...
   - {If problems found}: Fix the issues on the board, then re-run `/graph_ba_from_board analyze`
   - {If board is clean}: Board is ready for sync. Run `/graph_ba_from_board sync`
   - {If graph mismatches found}: Re-sync with `/graph_ba_from_board sync` to update
   ```
...
```
/graph_ba_from_board sync [board_path]
```
...
2. **Pre-sync validation.** Run a quick completeness check (Phase 2 of `graph_ba_analyze/SKILL.md`). If critical problems are found (isolated steps, elements without customData), warn the user:
   ...
   Wait for user confirmation before proceeding. If the user confirms, continue. If the user declines, stop and suggest `/graph_ba_from_board analyze` for the full report.

3. **Execute all 6 phases from `graph_ba_sync/SKILL.md`:**
...
4. **Append orchestrator footer:**
   ...
   - To verify: `/graph_ba_from_board status`
   - To re-analyze: `/graph_ba_from_board analyze`
   - To proceed to SA phase: `/graph_sa_architect`
   ```
...
```
/graph_ba_from_board status [board_path]
```
...
3. **Classify elements** using the same logic as `graph_ba_analyze/SKILL.md` Phase 1:
...
4. **Compute and report statistics:**
   ...
   **Estimated problems:** {N} (run `/graph_ba_from_board analyze` for details)

   ### Quick actions
   - Full analysis: `/graph_ba_from_board analyze`
   - Sync to graph: `/graph_ba_from_board sync`
   - Import more data: `/graph_ba_from_board import <file>`
   ```
...
```
BA: /graph_ba_from_board new orders
  -> Board created: {$boards_dir}/orders-board.excalidraw
  -> 3 swimlane placeholders

BA: /graph_ba_from_board import process-description.docx
...
BA: /graph_ba_from_board analyze
...
BA: /graph_ba_from_board status
  -> 10 elements, all synced, 0 problems
  -> "Data is in the graph. Proceed to /graph_sa_architect for SA phase."
```
...
```
/graph_ba_from_board enrich [board_path]
```
...
3. **Run enrichment skills sequentially** — each adds depth to the graph data created by sync:
   ...
   a. **graph_ba_entities** (COLLECT mode) — collect entities from READS/PRODUCES in graph, add attributes, states, stereotypes
   b. **graph_ba_roles** — consolidate roles, add departments, responsibilities, build matrix
   c. **graph_ba_rules** (full mode) — extract business rules from entity constraints and workflow conditions
   d. **graph_ba_glossary** (incremental mode) — create glossary terms for all named nodes
...
4. **Report enrichment results:**
   ...
   ### Next steps
   - Validate: `/graph_ba_from_board validate`
   - Proceed to handoff: `/graph_ba_from_board handoff`
   ```
...
```
/graph_ba_from_board validate [board_path]
```
...
2. **Delegate to graph_ba_validate** with scope `internal` (L1-L8).
...
```
/graph_ba_from_board handoff [board_path]
```
...
2. **Delegate to graph_ba_handoff** (full mode).
...
3. **Append orchestrator footer:**
   ...
   - Architecture: `/graph_sa_architect`
   - Full SA: `/graph_sa_full`
   ```
...
```
/graph_ba_from_board full <file_path>
```
...
| Situation | Response |
|-----------|----------|
| **No board found** (no argument, no session board, no files) | "No Excalidraw board found. Create one: `/graph_ba_from_board new <project_name>`" |
...
| **Sync: conflicts** (elements marked synced but missing in graph) | Surface the error from `graph_ba_sync/SKILL.md` Phase 5.1. Offer two options: (1) reset elements and re-sync as new, or (2) run `/graph_ba_from_board analyze` for graph comparison. |

---

## Shared References

Before executing any command, read `graph_core/SKILL.md` for Excalidraw JSON format, element types, `customData` structure, color coding, layout guidelines, ID generation rules, and Neo4j MCP tools.

Sub-skill references (read on demand when delegating):

| Skill | File | When |
|-------|------|------|
| Import | `graph_ba_import_doc/SKILL.md` | `import` command |
| Analyze | `graph_ba_analyze/SKILL.md` | `analyze`, `import`, `sync` commands |
| Sync | `graph_ba_sync/SKILL.md` | `sync` command |
| Entities | `graph_ba_entities/SKILL.md` | `enrich` command |
| Roles | `graph_ba_roles/SKILL.md` | `enrich` command |
| Rules | `graph_ba_rules/SKILL.md` | `enrich` command |
| Glossary | `graph_ba_glossary/SKILL.md` | `enrich` command |
| Validate | `graph_ba_validate/SKILL.md` | `validate` command |
| Handoff | `graph_ba_handoff/SKILL.md` | `handoff` command |
...
- `graph_core/SKILL.md`, sub-skill SKILL.md files (on demand)
...
- [ ] Delegates to `graph_ba_import_doc/SKILL.md` Phases 1-4
- [ ] Auto-triggers analysis (`graph_ba_analyze/SKILL.md` Phases 1-5)
- [ ] Combined report shown

### Command: `analyze`
- [ ] Delegates to `graph_ba_analyze/SKILL.md` Phases 1-5
- [ ] Orchestrator recommendations appended

### Command: `sync`
- [ ] Pre-sync validation (quick completeness check)
- [ ] User confirmation on critical problems
- [ ] Delegates to `graph_ba_sync/SKILL.md` Phases 1-6
