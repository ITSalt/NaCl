 ...
 * - The server marks the board dirty (hash mismatch), WS broadcasts board.changed,
 *   and the React store refreshes syncStatus to 'dirty'.
 * - We then assert the 🟡 dot appears in the sidebar.
 *
...
test.describe('Canvas', () => {
  ...
  test('editing the canvas via API marks the board as dirty (🟡)', async ({ page, request }) => {
    ...
    // After the PUT, the board's contentHash will diverge from contentHashAtLastSync
    // → syncStatus becomes 'dirty' → sidebar dot becomes 🟡.
    // The WS broadcast causes the React store to refresh the board list.
