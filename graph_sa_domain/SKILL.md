...
# /graph_sa_domain — Domain Model (Graph)
...
**When:** After `graph_sa_architect`, building full domain model for a module from scratch.
...
> ⚠️ Следующие сущности не имеют атрибутов: {list of entity_name}. Добавьте атрибуты через `/graph_sa_domain MODIFY <entity>` или `/graph_sa_domain CREATE`.
...
If module not found, suggest creating it first via `graph_sa_architect`.
