...
description: |
  ...
  run stub check, stub report, or the user says "/tl-stubs".
...
```
/tl-stubs                 # Full scan of src/, update registry, print summary
/tl-stubs UC###           # UC-specific: scan files from result artifacts only
/tl-stubs --final         # Pre-release: stricter thresholds, warnings block too
```
...
1. `.tl/` directory must exist (else: `Error: Run /tl-plan to initialize`)
...
**UC scan**: write `.tl/tasks/UC###/stub-report.md` using `tl-core/templates/stub-report-template.md`. Sections: frontmatter (task_id, scan_date, status, counts), summary, critical/warning/info/orphaned stubs, blocking status, recommendations, registry update note.
...
### Before Review (tl-review checks)
...
Action: Resolve CRITICAL stubs, then /tl-stubs UC###
```
...
Next: Resolve critical stubs, then /tl-stubs
```
...
Action: Resolve STUB-001, then /tl-stubs UC001
```
...
| Situation | Response |
|-----------|----------|
| No `.tl/` directory | `Error: Run /tl-plan to initialize` |
| No source files in scope | `Error: No source files found in src/` |
| UC result files missing | `Error: No result files for UC###. Run /tl-dev-be or /tl-dev-fe first` |
...
| Topic | Reference |
|-------|-----------|
| Stub tracking rules (full) | `tl-core/references/stub-tracking-rules.md` |
| Stub report template | `tl-core/templates/stub-report-template.md` |
| TL protocol | `tl-core/references/tl-protocol.md` |
| FE code patterns | `tl-core/references/fe-code-style.md` |
...
- **Gate PASSED**: `/tl-review UC### --be` or `/tl-review UC### --fe`
- **Gate BLOCKED**: Fix critical stubs, then `/tl-stubs UC###` again
- **Full scan clean**: `/tl-stubs --final` (pre-release check)
- **Full scan issues**: `/tl-stubs UC###` per UC with critical stubs
- **Overview**: `/tl-status --stubs`
