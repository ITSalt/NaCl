...
description: |
  ...
  activity diagram, формы UC, graph_sa_uc, stories, detail UC.
---

# /graph_sa_uc --- Use Case Registry + Detailing (Graph)
...
> All WorkflowSteps with stereotype "Автоматизируется" already have AUTOMATES_AS edges. No new UC candidates. Run `/graph_sa_uc list` to see existing UCs.
...
Next: run `/graph_sa_uc detail UC-101` to detail each UC.
```
...
> UseCase `{UC-ID}` not found in graph. Run `/graph_sa_uc stories` first to create UC nodes, or `/graph_sa_uc list` to see existing UCs.
...
>
> Fix: either add MAPS_TO edges or create missing DomainAttributes (via `/graph_sa_domain`).
...
Next: detail another UC with `/graph_sa_uc detail UC-{NNN}` or validate all with `/graph_sa_validate`.
```
...
> Следующие UC ещё не детализированы: {list of uc_id}. Вызовите `/graph_sa_uc detail <UC-ID>` для каждого.
...
Next actions:
  ...
  - Run `/graph_sa_uc detail UC-{NNN}` to detail a specific UC
```
...
> No automation scope found in graph. Ensure BA data is loaded. Run `/graph_ba_sync` to synchronize BA board with Neo4j, then re-run `/graph_sa_uc stories`.

### Missing domain model

If `detail` finds no DomainEntity realized from related BA entities:

> WARNING: No DomainEntities found for BA entities related to this UC. Form-domain mapping will be incomplete. Consider running `/graph_sa_domain` first to create the domain model.
