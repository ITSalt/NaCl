---
name: graph_ba_entities
description: |
  Catalog business entities in Neo4j: stereotypes, attributes, states, CRUD matrix.
  Graph-first equivalent of ba-entities --- business-level types only, no technical types.
  Use when: model entities with graph, create entity catalog, or the user says "/graph_ba_entities".
---

# /graph_ba_entities --- Business Entities (Graph)
...
```
/graph_ba_entities mode=FULL|CREATE|MODIFY|COLLECT
```
...
**When:** WorkflowStep nodes already exist (from `/graph_ba_workflow`), need to gather referenced entities into a consolidated list.
...
> No workflow steps found in Neo4j. Run `/graph_ba_workflow` first to decompose processes into steps, then return to `/graph_ba_entities`.

If no `BusinessProcess` nodes exist:

> No business processes found in Neo4j. Run `/graph_ba_process` first, then `/graph_ba_workflow`, then return to `/graph_ba_entities`.
...
2. Suggest next steps:
   ...
   Next steps:
   1. `/graph_ba_roles` --- detail roles and authorities
   2. `/graph_ba_rules` --- catalog business rules constraining entities
   3. `/graph_render` --- export diagrams to Excalidraw / Mermaid
   ```
...
Next steps:
1. `/graph_ba_entities CREATE` --- add another entity
2. `/graph_ba_entities MODIFY` --- adjust the new entity
```
...
Next step:
1. `/graph_ba_entities FULL` --- describe all entities (attributes, states, relationships, CRUD)
```
