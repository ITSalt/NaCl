...
description: |
  ...
  check acceptance criteria, or the user says "/tl-qa UC###".
---
...
| Command | Description |
|---------|-------------|
| `/tl-qa UC###` | Run E2E QA for a UC task |
...
If not met, report the current state and suggest `/tl-sync UC###` or `/tl-stubs UC###`.
...
Create `.tl/tasks/UC###/qa-report.md` using `tl-core/templates/qa-report-template.md`. Include: frontmatter with verdict and counts, scenario description, every test step with action/expected/actual/status/screenshot, acceptance criteria table, bug descriptions (if any), verdict with conditions, recommendation.
...
<body>
  ...
  <footer><p>Generated by /tl-qa | Claude Skills · {N} screenshots</p></footer>
</body>
</html>
```
...
|---------|----------|-------------|
| API returns wrong data or 500 error | Backend | `/tl-dev-be UC### --continue` |
| Form validation missing/incorrect | Frontend | `/tl-dev-fe UC### --continue` |
| UI does not display API data correctly | Frontend | `/tl-dev-fe UC### --continue` |
| Page not found (404 on route) | Frontend | `/tl-dev-fe UC### --continue` |
| CORS or network error | Backend | `/tl-dev-be UC### --continue` |
| Wrong calculation from API | Backend | `/tl-dev-be UC### --continue` |
| Wrong calculation in UI | Frontend | `/tl-dev-fe UC### --continue` |
...
| Page not found (404) | Screenshot, record FAIL, suggest `/tl-dev-fe UC### --continue` |
...
Next:
  PASS -> /tl-ship UC### (commit + push)
  FAIL -> /tl-dev-be UC### --continue  (backend fix)
         /tl-dev-fe UC### --continue  (frontend fix)
```
...
| Document | Purpose |
|----------|---------|
| `tl-core/references/qa-rules.md` | Complete QA rules and procedures |
| `tl-core/templates/qa-report-template.md` | QA report template with all sections |

## Next Steps

| Verdict | Next Action |
|---------|-------------|
| PASS | `/tl-full UC###` -- finalize the task |
| FAIL | `/tl-dev-be UC### --continue` or `/tl-dev-fe UC### --continue` -- fix and re-test |
