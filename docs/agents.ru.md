...
Запускать все 59 скиллов на Opus -- расточительство бюджета на задачах, с которыми Sonnet справляется одинаково хорошо. Запускать всё на Haiku -- потеря качества там, где важны рассуждения. Архитектура агентов маршрутизирует каждый скилл на правильную модель.
...
### developer -- Sonnet, medium effort (7 скиллов)

Генератор кода. Реализует фичи через TDD из спецификаций.

**Инструменты:** Read, Write, Edit, Grep, Glob, Bash

**Скиллы:** `nacl-tl-dev`, `nacl-tl-dev-be`, `nacl-tl-dev-fe`, `nacl-tl-fix`, `nacl-tl-regression-test`, `nacl-tl-docs`, `nacl-tl-reopened`
...
Есть два канала, как и у скиллов:

- **Claude Code Desktop (плагин):** `/plugin marketplace add ITSalt/NaCl`, затем `/plugin install nacl@nacl` -- устанавливает `plugin/agents/` (все 7 файлов агентов) вместе со скиллами плагина, без ручного симлинкования.
- **Claude Code CLI:** запустите `scripts/install-claude-code-skills.sh` -- он за один проход симлинкует и `.claude/agents/*.md` -> `~/.claude/agents/<name>`, и скиллы. Циклы ниже делают то же самое вручную, для случаев, когда запустить скрипт нельзя.
