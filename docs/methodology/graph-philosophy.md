...
**Living documentation.** The `nacl-publish` skill renders the graph into Docmost wiki pages. Excalidraw board generation is no longer performed by a skill in the render chain -- it now runs deterministically in the analyst-tool backend (`analyst-tool/server/src/render/excalidraw/`). The documentation is always derived from the graph -- it cannot fall out of sync because it is regenerated from the source of truth every time it is published. This inverts the traditional relationship between specs and documentation. Instead of documentation being the primary artifact that might or might not reflect reality, the graph is the primary artifact and documentation is a read-only projection of it.
