...
Это установит 53 из 59 скиллов как slash-команды `/nacl:<name>` и 7 профилей
агентов как `@nacl:<name>`. `nacl-goal` исключён (он оборачивает CLI-only
команду `/goal`, которую Desktop не может запустить); `nacl-postmortem` и три
скилла `nacl-migrate*` исключены как редкие/требующие checkout репозитория;
`nacl-core` исключён как shared-библиотека, поставляемая целиком в корне
плагина, а не как вызываемый скилл (всего 6 исключений: 59 − 6 = 53).
