...
4. Откройте новую задачу в каноническом корне проекта, доверьте проект по
   запросу Codex и запустите `nacl-init`.

Публичной карточки или install URL пока нет. После того как подтверждённый init
создаст launcher без секрета и объединит `[mcp_servers.nacl_neo4j]` в проектном
`.codex/config.toml`, откройте ещё одну новую задачу в том же каноническом
доверенном корне, чтобы Codex загрузил проектный `neo4j-mcp`; это не вторая
установка. Не используйте сохранённый путь к пакету с другого компьютера.
Проектный `.mcp.json` относится только к Claude/compatibility. Текущий immutable
...
Откройте установленную карточку NaCl в **Plugins** и выберите **Update**, когда
Codex его предложит. Если обновление не предложено, остановитесь и запросите у
владельца плагина нужный релиз; не выдумывайте install URL или путь к пакету.

Для официального Skills-only канала полностью перезапустите Codex, откройте
**новую задачу в том же каноническом доверенном корне**, вызовите установленный
`nacl-init` и выполните только его read-only preflight предпосылок/замыкания.
Если bootstrap уже создал проектный `.codex/config.toml`, проверьте
`nacl_neo4j` в новой задаче и повторите graph/schema read checks без изменения
данных. Не вызывайте `nacl_installation_doctor`: в Skills-only bundle нет
пакетного MCP doctor.

Для Git/full-plugin канала совместимости полностью перезапустите Codex,
создайте новую задачу и повторите пакетную doctor-проверку выше. Её
`pluginVersion` должен совпадать с обновлённой карточкой, а
