...
Существующие симлинки скиллов обновляются мгновенно при изменении файлов в
`$HOME\NaCl`. **Новые** директории скиллов из релиза автоматически не
подключаются — запустите тот же блок PowerShell ещё раз. Он идемпотентен:
существующие ссылки пересоздаются на тот же таргет, для любой новой `nacl-*`
директории с `SKILL.md` создаётся новая ссылка. Полная процедура обновления
для всех платформ:
