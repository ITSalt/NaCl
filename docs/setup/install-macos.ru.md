...
`nacl-init`. После подтверждённого bootstrap, создания launcher без секрета и
записи проектного `.codex/config.toml` откройте ещё одну новую задачу в том же
каноническом доверенном корне для загрузки проектного MCP. Проектный
`.mcp.json` относится только к Claude/compatibility. Checkout, терминал,
