...
- Детерминированный marketplace bundle:
  `$TMPDIR/nacl-wave7-w7c2-5ef6f72-marketplace.tar.gz`;
...
```bash
git archive --format=tar --prefix=nacl-wave7-w7c2/ --output="$TMPDIR/nacl-wave7-w7c2-5ef6f72-marketplace.tar" 5ef6f72 .agents/plugins/marketplace.json plugins/nacl
gzip -n -f "$TMPDIR/nacl-wave7-w7c2-5ef6f72-marketplace.tar"
shasum -a 256 "$TMPDIR/nacl-wave7-w7c2-5ef6f72-marketplace.tar.gz"
```
