...
- Детерминированный marketplace bundle:
  `$SYSTEM_TEMP/nacl-wave7-w7c2-5ef6f72-marketplace.tar.gz`;
...
В строке historical evidence выше `$SYSTEM_TEMP` — только нейтральное
обозначение системного временного каталога, а не shell-переменная. Runnable
последовательность ниже сама безопасно создаёт и очищает рабочий каталог.

```bash
work_dir=$(mktemp -d)
trap 'rm -rf "$work_dir"' EXIT
archive="$work_dir/nacl-wave7-w7c2-5ef6f72-marketplace.tar"
git archive --format=tar --prefix=nacl-wave7-w7c2/ --output="$archive" 5ef6f72 .agents/plugins/marketplace.json plugins/nacl
gzip -n -f "$archive"
...
1. Нажать deeplink ниже. В карточке `NaCl Local Candidate` нажать **Upgrade** и
   ...
   **Settings → Plugins → Plugins**, удалить только установленный `nacl` из
   ...
   `codex://plugins/nacl?marketplacePath=$URL_ENCODED_MARKETPLACE_PATH`
