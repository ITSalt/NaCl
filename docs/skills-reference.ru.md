...
| Скилл | Описание | Пример |
|-------|----------|--------|
| `nacl-goal` | Разрешает NaCl-алиас в условие завершения для `/goal`, которое может проверить transcript-only эвалюатор через протокол GOAL_PROOF. По умолчанию работает в режиме предпросмотра; `--start` запускает реальный `/goal`. Отказывает при попытке пройти через Tier-C ворота (BA-SA handoff, подтверждение фаз SA, hotfix). Документация: `docs/guides/goal-command.md`. | `/nacl-goal wave:5` |
