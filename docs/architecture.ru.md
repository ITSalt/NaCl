...
## Codex Skills-only Runtime и граница проекта

Публичная поставка Codex — один Skills-only bundle с десятью
скиллами-дирижёрами. Каждый загружаемый скилл содержит полное замыкание своих
workflow, скриптов, шаблонов и assets; в публичном artifact нет пакетного
MCP binding, app binding или зависимости от hosted MCP. Сгенерированный полный
плагин в `plugins/nacl/` остаётся Git-пакетом разработки и совместимости; его
inventory из шестидесяти workflow и 25 инструментов не является публичным
runtime-контрактом.

`nacl-init` — граница bootstrap. Он проверяет предпосылки, показывает точный
план без мутаций и действует только после свежего подтверждения. Его пакетные
скрипты создают или подключают отдельный Neo4j 5 Community stack проекта,
устанавливают запиненный и проверенный по checksum `neo4j-mcp` и объединяют
безопасную запись `mcp_servers` в проектном `.codex/config.toml`, не сохраняя
сырой секрет и не перезаписывая другие серверы. После записи файла текущая
задача останавливается; новая задача в том
же проекте обязана загрузить проектный MCP и проверить handshake, схему,
чтение, подтверждённую запись и read-back.

Neo4j остаётся отдельно администрируемым для каждого проекта — локально или на
доступном VPS. Сервер является границей авторизации: доступ к одному серверу
означает доступ ко всем базам проектов на нём, а другой сервер требует
отдельного grant. `project_scope` обеспечивает маршрутизацию и provenance, а
не авторизацию. Credentials разрешаются проектным локальным runtime и не
встраиваются в Skills-only bundle, коммиченный config, логи или аргументы
процесса. Корневой `.mcp.json` относится только к Claude или явно
документированным full-plugin compatibility-сценариям; это не файл подхвата
Codex Skills-only. NaCl не вводит публичный или managed MCP-сервис.
