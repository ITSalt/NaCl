...
- **Оркестратор** — запускает все шаги автоматически (`graph_ba_full`, `graph_sa_full`, `graph_tl_conductor`)
...
```
Уровень 4: graph_tl_conductor          (весь пайплайн: заявки → разработка → staging)
Уровень 3: graph_ba_full / graph_sa_full / tl-full   (оркестраторы уровня)
Уровень 2: graph_ba_context / tl-dev-be / tl-qa      (отдельные скиллы)
Уровень 1: graph_core / tl-core                       (общие справочники)
```
...
Входные данные (интервью, документы)
    ↓
graph_ba_* → Neo4j BA-узлы (процессы, сущности, роли, правила)
    ↓
graph_sa_* → Neo4j SA-узлы (модули, домен, UC, формы, API-контракты)
    ↓
graph_tl_plan → .tl/tasks/ (файлы задач со спецификациями из графа)
    ↓
tl-dev-be → src/ бэкенд-код (TDD: сначала тест, потом реализация)
tl-dev-fe → src/ фронтенд-код (TDD: тот же подход)
    ↓
...
5. **Ссылки** — на общие ресурсы в `graph_core/` или `tl-core/`
