---
name: graph_ba_handoff
description: |
  Generate BA→SA handoff package from Neo4j: traceability matrix, automation scope,
  module suggestions, coverage stats. Creates cross-layer edges after user confirmation.
  Graph-first equivalent of ba-handoff.
  Use when: prepare BA→SA handoff, build traceability matrix, or the user says "/graph_ba_handoff".
---

# /graph_ba_handoff --- BA→SA Handoff Package (Graph)
...
Если все счётчики = 0 --- предупреди, что BA-подграф пуст; предложи сначала `/graph_ba_process` или `/graph_ba_import_doc`.
...
Важно: это предложение. Финальную структуру модулей определяет /graph_sa_architect.
...
Если Module узла ещё нет --- **не создавай его здесь**. Запиши предложение в вывод для последующего `/graph_sa_architect`.
...
Следующие шаги:
1. /graph_sa_architect --- создать модули на основе предложений
2. /graph_sa_domain --- построить Domain Model для модулей
3. /graph_sa_uc --- детализировать Use Cases из кандидатов
4. /graph_sa_roles --- определить системные роли
```
...
3. Предложи пользователю:
   - `/graph_sa_architect` --- для создания модулей
   - `/graph_sa_domain` --- для создания DomainEntity
   - `/graph_sa_uc` --- для создания UseCase
   - `/graph_sa_roles` --- для создания SystemRole
4. После создания SA-узлов --- повторный запуск `/graph_ba_handoff` создаст рёбра

---

## Чеклист /graph_ba_handoff
