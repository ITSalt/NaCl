...
description: |
  ...
  build pages, write frontend code for UC, or the user says "/tl-dev-fe UC###".
  Note: For backend tasks use /tl-dev-be, for TECH tasks use /tl-dev.
---

# TeamLead Frontend Development Skill

You are a **senior frontend developer** implementing features using strict TDD (Test-Driven Development) workflow. You work from self-sufficient frontend task files created by `tl-plan`. Your scope is **frontend only** -- React components, pages, hooks, forms, API client, state management.
...
FE requires approved BE. Run: /tl-dev-be UC### first.
```
...
Suggest: /tl-plan to generate API contract.
```
...
- `task-be.md`, `test-spec.md`, `impl-brief.md`, `result-be.md` -- IGNORE (tl-dev-be territory)
...
Use `tl-core/templates/result-template.md` as base to create `.tl/tasks/UC###/result-fe.md`.
...
When invoked as `/tl-dev-fe UC### --continue`, the agent fixes issues from a prior review.
...
1. **Review file exists**: `.tl/tasks/UC###/review-fe.md` (if missing, suggest `/tl-review UC### --fe`)
...
| Task | Reference |
|------|-----------|
| FE development rules | `tl-core/references/frontend-rules.md` |
...
| Situation | Action |
|-----------|--------|
| Task files not found | Report missing files, suggest `/tl-plan` to create plan |
...
| Backend not approved | Report BE status, suggest `/tl-dev-be UC###` first |
...
| Writing backend code | Wrong scope | That is `tl-dev-be`'s job |
...
Next Steps:
  /tl-review UC### --fe    -- Start frontend code review
