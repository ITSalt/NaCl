...
description: |
  ...
  Task file format is IDENTICAL to tl-plan (dev agents don't change).
  Use when: create dev plan from graph, plan implementation, generate tasks,
  create development schedule, generate execution waves, or the user says "/graph_tl_plan".
---

# /graph_tl_plan -- Development Planning from Neo4j Graph

## Purpose

Graph-powered replacement for `/tl-plan`. Reads the SA specification from Neo4j
(modules, use cases, entities, dependencies) via Cypher queries and generates
self-sufficient task files for dev agents (`tl-dev-be`, `tl-dev-fe`).

**Critical difference from tl-plan:**

| Aspect | tl-plan | graph_tl_plan |
|--------|---------|---------------|
...
2. Suggest user runs `/graph_sa_architect` or `/graph_sa_domain` first.
...
| **SYNC after pair** | `tl-sync` runs when both BE and FE for a UC are approved |
...
**CRITICAL**: Task files must be **self-sufficient**. Dev agents (`tl-dev-be`, `tl-dev-fe`) must NOT query Neo4j or read SA docs during development. ALL information from the graph subquery must be embedded into the task files.
...
**Generated:** {date}
**Source:** Neo4j graph (graph_tl_plan)
...
|------|-------|-------|------------|
| UC001-BE | {title} API | tl-dev-be | TECH-001, TECH-003 |
| ... |

### Wave 2: Core Frontend + Dependent Backend
| Task | Title | Agent | Depends On |
|------|-------|-------|------------|
| UC001-FE | {title} Form | tl-dev-fe | UC001-BE |
| ... |
...
- Created development plan from Neo4j graph (graph_tl_plan)
...
1. Connection: read from config.yaml graph section (see graph_core/SKILL.md → Graph Config Resolution). MCP tools handle the connection automatically.
...
1. STOP immediately.
2. Report: "SA layer is empty. Run `/graph_sa_architect` or `/graph_sa_domain` to populate it."
