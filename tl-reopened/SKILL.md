...
description: |
  ...
  fixes via /tl-fix, ships via /tl-ship, closes the verification loop.
  Use when: fix reopened tasks, process QA failures, handle rework,
  or the user says "/tl-reopened".
---
...
You are the **feedback loop closer**. When `/tl-verify` or `/tl-qa` moves a task to the Reopened column (failed verification), you pick it up, understand what went wrong from tester feedback, orchestrate the fix, and push it back to DevDone. You close the gap between "verification failed" and "code is fixed."

## Key Principle

```
/tl-verify ─── FAIL ──→ Reopened column
                              │
/tl-reopened → /tl-fix → /tl-ship → DevDone
                                       │
                          /tl-verify (re-verify) → PASS → ToRelease
```

You do NOT write fixes yourself — you delegate to `/tl-fix` (spec-first bug fixing) and `/tl-ship` (commit + push). Your job is orchestration: fetch context, synthesize the problem, delegate, and report.
...
```
/tl-reopened                      # Interactive: list reopened tasks, user selects
/tl-reopened --all                # Process ALL reopened tasks sequentially
/tl-reopened --task ELE-644       # Process specific YouGile task by code
/tl-reopened UC028                # Process by UC ID (looks up in YouGile)
/tl-reopened --yes                # Skip USER GATE, auto-approve plans
/tl-reopened --auto-ship          # After fix, auto-ship (passes through to /tl-fix)
/tl-reopened --dry-run            # Fetch + context + plan only, no changes
```
...
3. **Parse messages for key markers** (search in order of priority):
   ...
   |--------|-------------|-----------------|
   | "VERIFICATION REPORT" or "Автоматическая верификация" | /tl-verify, /tl-verify-code | Verdict (PASS/FAIL), findings[], data flow issues |
   | "QA REPORT" or "qa-report" | /tl-qa | Failed scenarios, screenshots, acceptance criteria gaps |
   | "Отчёт разработки" or "Development report" | /tl-ship, previous /tl-reopened | What was already attempted, files changed |
   | Other comments | Analyst/PM/User | Additional context, priority notes, clarifications |

4. **Count previous rework iterations:**
   - Search messages for "tl-reopened" or "Reopened task picked up"
   - If count >= 2: **ESCALATE** — do not auto-fix. Present to user:
     ```
     ⚠️ Task {CODE} has been through tl-reopened {N} times already.
...
5. **Synthesize problem description** for `/tl-fix`:
   - Combine tester verdict, specific findings, file paths, and failed criteria
   - Format as a natural-language problem description (what `/tl-fix` expects as input)
   - Example: `"tl-verify found FAIL: API endpoint POST /api/orders returns 500 when order.items is empty (file: src/routes/orders.ts:45). Acceptance criterion AC-3 not met: 'empty cart should show validation error, not crash'."`
...
- Task was likely created directly in YouGile (e.g., by /tl-qa bug auto-creation, or by a human)
- Context comes entirely from YouGile chat messages (Step 2)
- `/tl-fix` will handle this via its L3 path (no docs for area)
...
═══════════════════════════════════════════════════════
  ...
  Fix approach:
    → Delegate to /tl-fix "{synthesized description}"
    → /tl-fix will classify (L0/L1/L2/L3) and apply spec-first fix
    → Post-fix: /tl-review + /tl-stubs quality gates
    → Ship: /tl-ship → DevDone
...
2. Post pickup message to task chat:
   ```
   send_task_message(taskId, "🔄 Reopened task picked up for rework by /tl-reopened.
   Problem: {brief summary from Step 2}.
   Approach: /tl-fix → /tl-review → /tl-stubs → /tl-ship")
   ```
...
**Goal:** Fix the issue by delegating to `/tl-fix`.

Invoke `/tl-fix` via Skill tool with the synthesized problem description from Step 2:
...
- If `--auto-ship` was passed to `/tl-reopened`: add `--auto-ship`
- If Path A and fix is clearly code-only (tester found a specific code bug, no spec drift): consider `--l1`

**Wait for /tl-fix to complete.** Capture its output:
- Fix level (L0/L1/L2/L3)
- Files changed
- Tests added/updated
- Validation result (tests pass / build OK)

**If /tl-fix fails** (tests don't pass, build broken, cannot determine fix):
...
- `/tl-fix` already includes a validation step (Step 7 in its workflow)
...
**Retry loop:** If review rejects, fix issues and re-review. Max 2 retries within tl-reopened (since /tl-fix already iterated internally). After 2 rejections:
...
**If /tl-fix already shipped via `--auto-ship`:** Skip to the DevDone move.

**Otherwise:**
```
Skill: tl-ship
Args: "{task_code} fix: {brief description from tl-fix report}"
```
...
**If /tl-ship was NOT invoked** (e.g., /tl-fix auto-shipped), move to DevDone manually:
```
...
📊 Уровень фикса: {L0/L1/L2/L3 from tl-fix}
...
🧪 Тесты:
- {test results summary from tl-fix report}

📁 Изменённые файлы:
- {file list from tl-fix report}
...
═══════════════════════════════════════════════════════
  ...
  Next step:
    /tl-verify {task_code}    # re-verify the fix
═══════════════════════════════════════════════════════
```
...
Next steps:
  /tl-verify --all    # re-verify all fixed tasks
```
   ...
   For each task:
     Launch Task agent:
       "Process reopened task {CODE}: {synthesized description}.
        ...
        Run: /tl-fix → /tl-review → /tl-stubs → /tl-ship"
...
| Condition | Action |
|-----------|--------|
| Task has been through /tl-reopened 2+ times | Escalate to user, do not auto-fix |
| /tl-fix cannot determine root cause | Post analysis to YouGile, escalate to user |
...
Skip all column movements and chat posting. Accept problem description from user as input (like `/tl-fix`). Report results locally only.

### No .tl/ directory
Path B workflow. `/tl-fix` handles the "no docs" case via its L3 classification.
...
- Run `/tl-sync` after both fixes if UC context exists (Path A)

### Task is a TECH task (not UC)
If the task has no UC association:
- Path B applies
- Use `/tl-fix` as usual — it handles infrastructure bugs too
- Skip `/tl-review --be/--fe` flags, use plain `/tl-review`

---

## Without YouGile (Manual Mode)

If the user invokes `/tl-reopened` without arguments and YouGile is not configured:

```
No YouGile configured. To process a reopened task manually, describe the problem:

  /tl-reopened "POST /api/orders returns 500 when cart is empty"

This will delegate to /tl-fix with your description.
```

The skill then runs Steps 5-10 with:
- Step 5: Plan based on user description
- Steps 6, 9 (YouGile): skipped
- Step 7: `/tl-fix "{user description}"`
- Step 8: Review + stubs
...
- `tl-fix/SKILL.md` — primary delegation target (8-step spec-first bug fix)
- `tl-verify/SKILL.md` — upstream: sends FAIL tasks to Reopened (Step 6)
- `tl-ship/SKILL.md` — downstream: commit + push + PR + YouGile
- `tl-review/SKILL.md` — quality gate (code review)
- `tl-stubs/SKILL.md` — quality gate (stub scanning)
- `tl-core/references/tl-protocol.md` — agent contracts
- `tl-core/templates/config-yaml-template.yaml` — config.yaml reference (has `reopened` column)
