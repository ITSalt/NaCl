...
name: graph_core
description: |
  Shared references, templates, and utilities for all graph_* skills.
...
This skill is NOT invocable by users. It provides shared references, templates, and utilities for all `graph_*` skills.
...
| Skill | Purpose |
|-------|---------|
| graph_ba_context | System boundaries → Neo4j |
| graph_ba_process | Business process map → Neo4j |
| graph_ba_workflow | Activity diagrams → Neo4j |
| graph_ba_entities | Entity catalog → Neo4j |
| graph_ba_roles | Business roles → Neo4j |
| graph_ba_glossary | Glossary → Neo4j |
| graph_ba_rules | Business rules → Neo4j |
| graph_ba_validate | L1-L8 + XL1-XL5 validation |
| graph_ba_handoff | BA→SA traceability |
| graph_ba_full | Full BA orchestrator (10 phases) |
| graph_ba_from_board | Board orchestrator (import/sync/enrich/validate/handoff) |
| graph_ba_import_doc | Document → Excalidraw |
| graph_ba_analyze | Board completeness analysis |
| graph_ba_sync | Board → Neo4j sync |

### SA Layer (9 skills)
| Skill | Purpose |
|-------|---------|
| graph_sa_architect | Module decomposition |
| graph_sa_domain | Domain model |
| graph_sa_roles | Role model + permissions |
| graph_sa_uc | UC registry + detailing |
| graph_sa_ui | UI architecture |
| graph_sa_validate | L1-L6 + XL6-XL9 validation |
| graph_sa_feature | Incremental feature specification |
| graph_sa_finalize | Statistics, ADR, readiness |
| graph_sa_full | Full SA orchestrator (10 phases) |

### TL Layer (6 skills)
| Skill | Purpose |
|-------|---------|
| graph_tl_plan | Task planning from graph |
| graph_tl_intake | Graph-aware triage |
| graph_tl_status | Status + SA coverage |
| graph_tl_next | Next task + SA context |
| graph_tl_full | Full lifecycle orchestrator |
| graph_tl_conductor | Batch process manager |

### Output (2 skills)
| Skill | Purpose |
|-------|---------|
| graph_render | Graph → Markdown + Excalidraw |
| graph_publish | Graph → Docmost |
...
```json
{
  "customData": {
    ...
    "synced": false               // true after /ba-sync
  }
}
```
