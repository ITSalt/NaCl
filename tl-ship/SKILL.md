...
```
/tl-ship UC028                    # ship specific UC
/tl-ship --feature FR-001         # ship all tasks in a feature
/tl-ship "custom commit message"  # ship with explicit message
/tl-ship                          # ship all uncommitted changes (auto-compose message)
/tl-ship --deploy                 # ship + deploy (method from config.yaml)
/tl-ship UC028 --deploy           # ship UC + deploy
```
...
- [ ] E2E verification via /tl-verify
...
ELSE (feature/staging branch):
  ...
  ELSE IF method == "github-actions":
    ...
    (Optionally monitor with /tl-deploy --staging)
...
Next step:
  /tl-verify UC028  — verify implementation
═══════════════════════════════════════════════
```
...
- `tl-core/references/commit-conventions.md` — commit message format
