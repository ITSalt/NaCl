...
```
  ...
  │  Phase 0: INIT                                                       │
  │  ┌──────────┐                                                        │
  │  │ nacl-tl-plan  │ --> master-plan.md, Execution Waves, task files        │
  │  └──────────┘                                                        │
  │       │                                                              │
  │       ▼                                                              │
  │  Phase 1: INFRA (Wave 0)                                             │
  │  ┌──────────┐                                                        │
  │  │ nacl-tl-dev   │ --> TECH-001: Docker, DB, Redis                       │
  │  └──────────┘                                                        │
  │       │                                                              │
  │       ▼                                                              │
  │  Phase 2: UC DEVELOPMENT (Wave 1 -> Wave 2)                          │
  │  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐          │
  │  │nacl-tl-dev-be │   │nacl-tl-dev-fe │   │ nacl-tl-sync  │   │ nacl-tl-stubs │          │
  │  │ (BE TDD) │   │ (FE TDD) │   │(contract)│   │ (scan)   │          │
  │  └──────────┘   └──────────┘   └──────────┘   └──────────┘          │
  │       │               │              │               │               │
  │       ▼               ▼              ▼               ▼               │
  │  ┌──────────┐   ┌──────────┐   ┌──────────┐                         │
  │  │nacl-tl-review │   │nacl-tl-review │   │  nacl-tl-qa   │                         │
  │  │  --be    │   │  --fe    │   │  (E2E)   │                         │
  │  └──────────┘   └──────────┘   └──────────┘                         │
  │       │               │              │                               │
  │       │          REJECTED?           │                               │
  ...
  │  ┌──────────┐   ┌──────────┐   ┌──────────┐                         │
  │  │ nacl-tl-docs  │   │nacl-tl-status │   │ nacl-tl-next  │                         │
  │  └──────────┘   └──────────┘   └──────────┘                         │
  │                                                                      │
  └──────────────────────────────────────────────────────────────────────┘
```
...
## Phase 1: INFRA -- Wave 0 (`nacl-tl-dev TECH-001`)

### 1.1 Trigger

```
User: /nacl-tl-dev TECH-001
```

### 1.2 Что делает nacl-tl-dev
...
### Step 2a: Backend TDD (`nacl-tl-dev-be UC001`)

#### Trigger

```
User: /nacl-tl-dev-be BE-UC001
```

#### Что делает nacl-tl-dev-be
...
### Step 2b: Frontend TDD (`nacl-tl-dev-fe UC001`)

#### Trigger

```
User: /nacl-tl-dev-fe FE-UC001
```

#### Что делает nacl-tl-dev-fe
...
### Step 2c: Sync Verification (`nacl-tl-sync UC001`)

#### Trigger

```
User: /nacl-tl-sync UC001
```

#### Что делает nacl-tl-sync
...
nacl-tl-sync UC001
  ...
  --> Можно передавать в nacl-tl-review
```
...
### Step 2d: Stub Scanning (`nacl-tl-stubs UC001`)

#### Trigger

```
User: /nacl-tl-stubs UC001
```

#### Что делает nacl-tl-stubs
...
### Step 2e: Backend Review (`nacl-tl-review UC001 --be`)

#### Trigger

```
User: /nacl-tl-review UC001 --be
```

#### Что делает nacl-tl-review --be
...
### Step 2f: Frontend Review (`nacl-tl-review UC001 --fe`) -- REJECTED

#### Trigger

```
User: /nacl-tl-review UC001 --fe
```

#### Что делает nacl-tl-review --fe
...
nacl-tl-review UC001 --fe
  ...
  Verdict: REJECTED
  --> Возвращаем в nacl-tl-dev-fe для доработки
```
...
#### Fix: nacl-tl-dev-fe UC001 --continue

```
User: /nacl-tl-dev-fe FE-UC001 --continue
```
...
#### Re-review: nacl-tl-review UC001 --fe
...
### Step 2g: E2E Testing (`nacl-tl-qa UC001`)

#### Trigger

```
User: /nacl-tl-qa UC001
```

#### Что делает nacl-tl-qa
...
### Step 2h: Documentation (`nacl-tl-docs UC001`)
...
```
10:00  [nacl-tl-plan]      Planning started
                      - 4 tasks created (2 TECH, 1 BE, 1 FE)
                      - 3 Execution Waves defined
                      - .tl/ structure created

10:15  [nacl-tl-dev]       TECH-001 Docker Compose
       - 10:30        - PostgreSQL + Redis configured
                      - Status: done

10:30  [nacl-tl-dev]       TECH-002 Shared Types & Config
       - 10:50        - tsconfig, shared types, project structure
                      - Status: done

11:00  [nacl-tl-dev-be]    BE-UC001 development started
                      - Status: in_progress

11:00  [nacl-tl-dev-be]    RED phase
       - 11:15        - 9 tests written, all FAILING

11:15  [nacl-tl-dev-be]    GREEN phase
       - 11:55        - OrderService, Controller, Repository
                      - All 9 tests PASSING

11:55  [nacl-tl-dev-be]    REFACTOR phase
       - 12:15        - Code improved, tests GREEN
                      - Status: ready_for_review

12:15  [nacl-tl-dev-fe]    FE-UC001 development started
                      - Status: in_progress

12:15  [nacl-tl-dev-fe]    RED phase
       - 12:35        - 11 RTL tests written, all FAILING

12:35  [nacl-tl-dev-fe]    GREEN phase
       - 13:25        - Components, API client, hooks
                      - All 11 tests PASSING

13:25  [nacl-tl-dev-fe]    REFACTOR phase
       - 13:50        - a11y, memo, hook extraction
                      - Status: ready_for_review

14:00  [nacl-tl-sync]      UC001 sync check
       - 14:15        - 12 pass, 0 fail, 2 warnings
                      - Verdict: PASS_WITH_WARNINGS

14:15  [nacl-tl-stubs]     UC001 stub scan
                      - 2 stubs found (medium, low)
                      - No critical -> proceed

14:30  [nacl-tl-review]    BE-UC001 review (--be)
       - 14:45        - Verdict: APPROVED (1st attempt)

14:45  [nacl-tl-review]    FE-UC001 review (--fe)         <-- FIRST ATTEMPT
       - 15:00        - Verdict: REJECTED
                      - Blocker: missing ErrorBoundary

15:00  [nacl-tl-dev-fe]    FE-UC001 fix (--continue)      <-- RETRY
       - 15:20        - ErrorBoundary added
                      - 12/12 tests pass
                      - Status: ready_for_review

15:20  [nacl-tl-review]    FE-UC001 re-review (--fe)      <-- SECOND ATTEMPT
       - 15:35        - Verdict: APPROVED

15:35  [nacl-tl-qa]        UC001 E2E testing
       - 15:50        - 6/6 steps PASS
                      - Screenshots saved

15:50  [nacl-tl-docs]      UC001 documentation
       - 16:00        - API docs, README, CHANGELOG updated
                      - Status: done

16:00  [nacl-tl-stubs]     Final stub check
                      - 0 critical stubs

16:00  [nacl-tl-status]    Final project status
                      - 4/4 tasks done (100%)
```
...
Каждый слой разрабатывается отдельным агентом (`nacl-tl-dev-be` / `nacl-tl-dev-fe`), ревьюится отдельно (`--be` / `--fe`), и результаты хранятся в отдельных файлах (`result-be.md` / `result-fe.md`).

### 2. API-контракт как источник истины

`api-contract.md` создаётся на этапе планирования и служит эталоном для:
- `nacl-tl-dev-be` -- реализует endpoints по контракту
- `nacl-tl-dev-fe` -- вызывает endpoints по контракту
- `nacl-tl-sync` -- проверяет соответствие обоих слоёв контракту
...
REJECTED (review-fe.md: "missing ErrorBoundary")
    │
    ▼
nacl-tl-dev-fe --continue (reads review-fe.md, fixes issue)
    │
    ▼
nacl-tl-review --fe (re-reviews -> APPROVED)
```
...
`nacl-tl-sync` запускается **после** обоих `nacl-tl-dev-be` и `nacl-tl-dev-fe`, но **перед** `nacl-tl-review`. Если sync находит BLOCKER -- review не начинается.

```
nacl-tl-dev-be --> nacl-tl-dev-fe --> nacl-tl-sync --> nacl-tl-review
                                 │
                            BLOCKER? --> nacl-tl-dev (fix) --> nacl-tl-sync (retry)
                            WARN?   --> continue to nacl-tl-review (with notes)
                            PASS?   --> continue to nacl-tl-review
```

### 5. Stub Safety Net

`nacl-tl-stubs` запускается после каждой фазы разработки и перед каждым ревью. Если в `stub-registry.json` есть записи с severity `high` или `critical` -- ревью автоматически REJECTED (Stub Gate).

### 6. E2E как финальная проверка

`nacl-tl-qa` запускается только когда и BE, и FE одобрены ревью. Он проверяет полный user flow через Playwright MCP.
