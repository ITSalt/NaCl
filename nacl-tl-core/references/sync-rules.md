# Sync Rules (nacl-tl-sync)

## 1. Назначение

Скилл `nacl-tl-sync` выполняет верификацию синхронизации между backend (BE) и frontend (FE) кодом на основе API-контракта. Цель --- убедиться, что BE реализует именно то, что описано в `api-contract.md`, а FE потребляет API именно так, как оно реализовано.

### Какие проблемы предотвращает

| Проблема | Последствия | Как nacl-tl-sync ловит |
|----------|------------|-------------------|
...
```
nacl-tl-plan → nacl-tl-dev(BE) → nacl-tl-dev(FE) → nacl-tl-sync → nacl-tl-review
                                        ↑
                                  Запускается после
                                  завершения BE и FE
```
...
```
┌─────────┐     ┌──────────┐     ┌──────────┐     ┌─────────┐     ┌───────────┐
│ nacl-tl-plan │ ──→ │ nacl-tl-dev   │ ──→ │ nacl-tl-dev   │ ──→ │ nacl-tl-sync │ ──→ │ nacl-tl-review │
│         │     │ (BE)     │     │ (FE)     │     │         │     │           │
...
└─────────┘     └──────────┘     └──────────┘     └─────────┘     └───────────┘
                                          ...
                                          review        nacl-tl-review
```

### Правила блокировки

1. **Если есть BLOCKER**: `nacl-tl-review` НЕ запускается. Возвращаем задачу в `nacl-tl-dev` для исправления
2. **Если только WARNING**: `nacl-tl-review` запускается, но warnings включаются в review checklist
3. **Если только INFO или чисто**: `nacl-tl-review` запускается нормально
...
sync-report.verdict = "FAIL" (есть BLOCKER):
  ...
  → nacl-tl-dev перечитывает sync-report.md и исправляет
...
После исправлений `nacl-tl-sync` запускается повторно:

```
nacl-tl-dev (fix) → nacl-tl-sync (re-check) → PASS? → nacl-tl-review
                                   → FAIL? → nacl-tl-dev (fix again)
```
