...
### 8. Дефекты в глобальных скиллах (выноси и жди)

Агенту **разрешено и поощряется** заглядывать за пределы файлов проекта — в том числе в сами файлы скиллов/фреймворка. Если во время работы над задачей ты обнаружил **противоречие или дефект в глобальном (фреймворочном) скилле** — например, захардкоженное значение, которое дублирует и теперь расходится с `config.yaml` (типичный случай: имя ветки `develop`/`main`, вписанное прямо в скилл вместо чтения из `config.yaml → git.main_branch` / `modules.[name].git_base_branch`) — **остановись, покажи пользователю точное противоречие (файл, строки, оба конфликтующих значения) и жди указаний.**

- **НЕ** правь глобальный скилл автономно — это непроверенное изменение фреймворка.
- **НЕ** продолжай молча, как будто противоречия нет.
- Правки глобальных скиллов выполняются **централизованно, с явного согласия пользователя**.

Вынести такую находку — это успех, а не выход за рамки: смотреть и выносить на обсуждение можно всегда; молча менять глобальный скилл — нельзя. Авторская сторона того же правила (как НЕ хардкодить ветку при написании скилла) — в `docs/configuration.md`, раздел «Branch-name discipline».
