...
## Применимость — Stack profile: Node.js / TypeScript (React)

Этот чеклист — **reference-профиль** для проектов, чей стек (по `config.yaml` → `modules.<name>.stack`) — React + TypeScript. Это не общефреймворковый мандат: NaCl не предписывает технологический стек. Для другой FE-экосистемы применяй базовые принципы ревью, адаптируя конкретные проверки. Источник истины о стеке проекта — `config.yaml`, а не этот профиль.
