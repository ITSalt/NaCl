...
- The source skill's `--autonomous` flag (2.13+) is a wrapper-only contract
  ...
  consolidated pre-start question — hard-refuse triggers always still stop.
  Codex sessions are interactive by definition: never honor `--autonomous`
  here; the interactive gate calibration above is the Codex behavior.
