...
| `repo-checks-GREEN:<commit>` | Repo-wide gate evidence written by `nacl-tl-review` when the project's resolved repo-wide lint, typecheck, and test commands all exit 0 on the wave-tip commit. The triple resolves per stage: config `repo_checks.<stage>` > package-manager autodetect (root `packageManager` field, else lockfile: pnpm → `pnpm -r <stage>`, npm → `npm run <stage> --workspaces`, yarn → `yarn workspaces run <stage>`) > unrunnable. `<commit>` is the full or short SHA of that commit (forward slash not used; the sha follows `:` directly). This evidence is required for `nacl-tl-review` to emit Status `VERIFIED`; absence of a recorded repo-checks run is `NOT_RUN`, which is not VERIFIED-equivalent downstream. May be recorded alongside a `test-GREEN:` value (separated by a single space) when both apply. |
...
- Prefix `repo-checks-GREEN:` → `repo-checks-GREEN` (commit SHA extracted for the report). A valid `repo-checks-GREEN:<commit>` value identifies the wave-tip commit on which the project's resolved repo-wide lint, typecheck, and test commands (config `repo_checks.*` or package-manager autodetect) all exited 0 — it is the only shape that makes the entry valid.
