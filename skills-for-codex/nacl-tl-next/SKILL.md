...
- Remote mode (`config.yaml` `graph.mode: remote`, shared graph): the `.tl/status.json` fallback in
  ...
  state). Recommendation is claim-first: claim the task atomically
  (`nacl-core/scripts/claim-task.mjs claim`) before presenting it; if another developer owns it, skip
  to the next candidate. See `../../nacl-tl-core/references/remote-mode-coordination.md`.
