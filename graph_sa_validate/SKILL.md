...
# /graph_sa_validate -- Specification Validation (Graph)
...
```
/graph_sa_validate [level] [--scope=<scope>]
```
...
| `--scope` | `intra-uc UC-NNN[,UC-NNN]` | Limit validation to specific UCs and their subgraph (forms, fields, requirements, entities). Used by graph_sa_feature for incremental validation. |
...
2. Suggest the user runs `/graph_sa_architect` or `/graph_sa_domain` first.
3. Explain that `/graph_sa_validate` works only with a populated Neo4j graph.
...
- `level=ba-cross` --> STOP, report that BA layer is not populated. User must run `/graph_ba_import_doc` or `/graph_ba_from_board` first.
...
```markdown
# Validation Report -- graph_sa_validate
...
- [ ] Re-run `/graph_sa_validate` after fixes
```
...
1. STOP immediately.
2. Report: "Graph is empty. Run `/graph_sa_architect` or `/graph_sa_domain` to populate the SA layer."
