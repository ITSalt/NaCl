...
description: |
  ...
  Invocation: /graph_ba_import_doc <file_path>
---

# /graph_ba_import_doc --- Import Document to Excalidraw Board
...
**You do NOT write to Neo4j.** You only produce an `.excalidraw` file. Syncing to the graph database is the responsibility of `/graph_ba_sync`.
...
- **`graph_core/SKILL.md`** --- Excalidraw JSON format, element types, color coding, customData structure, layout guidelines, ID generation rules.
...
| Level | Criteria | strokeColor (from graph_core) |
|-------|----------|-------------------------------|
...
This ensures graph_ba_analyze can detect PERFORMED_BY relationships via containment.
...
Every visual element consists of a **shape** (rectangle/diamond) plus a **bound text** element. Reference `graph_core/SKILL.md` for the canonical template.
...
> **Containment rule:** Steps MUST be positioned within their swimlane's bounding box (step.x >= swimlane.x, step.x + step.width <= swimlane.x + swimlane.width, step.y >= swimlane.y, step.y + step.height <= swimlane.y + swimlane.height). The `graph_ba_sync` skill relies on containment to infer PERFORMED_BY relationships.
...
For each step, create a rectangle with bound text. Apply colors from `graph_core/SKILL.md`:
...
The `strokeColor` is determined by **confidence** (see graph_core):
...
2. After review, run `/graph_ba_analyze` to validate the board structure.
3. When ready, run `/graph_ba_sync` to push elements to Neo4j.
```
...
This skill produces ONLY an `.excalidraw` file. It sets `"nodeId": null` and `"synced": false` on every element. Writing to Neo4j is the exclusive responsibility of `/graph_ba_sync`.
...
### 4. Colors come from graph_core

Do not invent colors. Use only the backgroundColor and strokeColor values defined in `graph_core/SKILL.md`:
...
| `graph_core/SKILL.md` | Read | Excalidraw format, customData, colors, layout rules |
...
| Caller | Context |
|--------|---------|
| User | Manual invocation: `/graph_ba_import_doc <file_path>` |
...
- [ ] Colors match `graph_core/SKILL.md` exactly
