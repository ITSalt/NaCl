---
name: graph_tl_conductor
description: |
  Graph-aware batch process manager: intake to staging.
  Delegates planning to graph_tl_plan, dev to graph_tl_full.
  Graph-first equivalent of tl-conductor.
  Use when: batch workflow with graph, orchestrate graph intake, or the user says "/graph_tl_conductor".
---
...
**Key advantage over tl-conductor:** Phase 0 can read UC scope directly from the Neo4j graph. Phase 2 delegates to `graph_tl_plan` (one Cypher query per UC instead of reading ~70 markdown files). Phase 3 delegates to `graph_tl_full` (graph-aware lifecycle executor).
...
```
/graph_tl_conductor --items FR-001,FR-002,BUG-003    # batch items from intake
/graph_tl_conductor --feature FR-001                  # single feature request
/graph_tl_conductor --branch feature/sprint-42        # explicit branch name
/graph_tl_conductor --skip-deliver                    # dev only, no delivery
/graph_tl_conductor --skip-qa                         # skip pre-ship QA in dev cycle
/graph_tl_conductor --yes                             # skip user gates
```
...
**If query returns empty or Neo4j unavailable:** Fall back to reading `.tl/feature-requests/FR-NNN.md` and extracting affected UCs from the markdown (same as tl-conductor).
...
**Critical difference from tl-conductor:** Delegates to `graph_tl_plan` instead of `tl-plan`.

For each FR item that needs planning:
1. Launch sub-agent (Task tool): `/graph_tl_plan --feature FR-NNN`
...
**Critical difference from tl-conductor:** Delegates UC lifecycle to `graph_tl_full` instead of `tl-full`.
...
2. Launch sub-agent (Task tool):
   Execute /graph_tl_full --task UC### --skip-plan [--skip-qa if conductor has --skip-qa]
...
Problems:
  1. UC029 failed at sync phase after 3 retries.
     ...
     Fix manually: /graph_tl_full --task UC029

Next:
  /tl-release              -- production release
  /graph_tl_full --task UC029    -- fix failed UC
  /graph_tl_conductor --items ... -- next batch
===============================================================
```
...
|-----------|-----------------|
| graph_tl_full returns DONE for UC | Commit atomically, update Task node in graph, continue |
| graph_tl_full returns FAILED for UC | Record failure with phase + reason, update Task node, continue to next UC |
...
| Neo4j unavailable at Phase 3 | graph_tl_full handles its own fallback; conductor continues normally |
...
phase = "development":
  ...
  - For UC: delegate to graph_tl_full --task (it has its own resumption via status.json)
...
|-------|------------------|-------|
| Phase 0 (init) | UserRequests -> Backlog | graph_tl_intake (before conductor) |
| Phase 3 (dev start) | Backlog -> InWork | conductor (or graph_tl_full internally) |
| Phase 3 (dev done) | InWork -> DevDone | graph_tl_full (per UC, automatic) |
...
```
graph_tl_intake -> classifies requests (graph-aware) -> recommends /graph_tl_conductor
graph_tl_conductor -> orchestrates batch -> delegates to:
  +-- /graph_tl_plan (graph-based planning, if needed)
  +-- /tl-dev TECH-### (infrastructure -- unchanged)
  +-- /tl-review TECH-### (TECH review -- unchanged)
  +-- /graph_tl_full --task UC### (graph-aware per-UC lifecycle)
  +-- /tl-fix "description" (bug fixes -- unchanged)
  +-- /tl-stubs --final (quality gate -- unchanged)
  +-- /tl-deliver (push -> verify -> deploy -- unchanged, format-agnostic)

graph_tl_full remains the per-UC lifecycle executor (graph-aware).
tl-deliver handles the ship->verify->deploy chain (format-agnostic, unchanged).
graph_tl_conductor is the process manager that ties everything together.
```

### Differences from tl-conductor

| Phase | tl-conductor delegates to | graph_tl_conductor delegates to |
|-------|---------------------------|--------------------------------|
| Phase 0 INIT | `.tl/feature-requests/FR-NNN.md` | Neo4j graph (fallback: FR-NNN.md) |
| Phase 2 PLAN | `/tl-plan` | `/graph_tl_plan` |
| Phase 3 DEV (UC lifecycle) | `/tl-full` | `/graph_tl_full` |
| Phase 3 DEV (TECH) | `/tl-dev` | `/tl-dev` (unchanged) |
| Phase 3 DEV (BUG) | `/tl-fix` | `/tl-fix` (unchanged) |
| Phase 4 QUALITY GATE | `/tl-stubs` | `/tl-stubs` (unchanged) |
| Phase 5 DELIVERY | `/tl-deliver` | `/tl-deliver` (unchanged) |

### What conductor does NOT do:
- Write or review code (delegates to tl-dev-be/fe via graph_tl_full)
- Run tests (delegates to graph_tl_full, tl-verify)
- Create specifications (delegates to graph_sa_feature via graph_tl_intake)
- Move YouGile tasks (delegates to sub-skills)
- Query Neo4j during development (graph_tl_full and graph_tl_plan handle that)
...
2. Read `.tl/feature-requests/FR-NNN.md` instead (same as tl-conductor)
3. Continue the workflow normally -- graph unavailability does NOT block orchestration
4. Sub-agents (graph_tl_plan, graph_tl_full) handle their own Neo4j fallback
...
- `graph_core/SKILL.md` -> Neo4j connection, schema, query library
