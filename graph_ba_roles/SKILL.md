...
**Important:** Business roles are organizational units (e.g. Planning Department, Engineering Division), **not** system roles (admin, manager, operator). System roles belong to SA (`/graph_sa_roles`).
...
**When:** After `/graph_ba_process` and `/graph_ba_workflow`, when building the role model for the first time.
...
1. Query Neo4j for `BusinessProcess` nodes --- if none exist, suggest running `/graph_ba_process` first:
...
> No business processes found in the graph. Run `/graph_ba_process` and `/graph_ba_workflow` first, then return to `/graph_ba_roles`.
...
> No BusinessProcess nodes found in Neo4j. Run `/graph_ba_process` first to create the process map, then return to `/graph_ba_roles`.
...
Next steps:
1. `/graph_ba_rules` --- catalogue business rules
2. `/graph_ba_glossary` --- build the domain glossary
3. `/graph_ba_validate` --- validate consistency of all BA artifacts
```

---

## Checklist /graph_ba_roles
