---
name: graph_ba_context
description: |
  Define system boundaries in Neo4j graph: scope, stakeholders, external entities, data flows.
  Graph-first equivalent of ba-context — all data stored as graph nodes and edges.
  Use when: define system scope with graph, system context diagram, or the user says "/graph_ba_context".
---

# /graph_ba_context — System Context Diagram via Neo4j Graph
...
This is the graph-first equivalent of `/ba-context`. The interactive protocol is identical; only the storage backend changes.
...
```yaml
agent: graph_ba_context
trigger: /graph_ba_context
...
2. Suggest next steps:
   ...
   Next steps:
   1. /graph_ba_process — define business processes in the graph
   2. /graph_ba_full — run full BA cycle in graph mode
   ```
