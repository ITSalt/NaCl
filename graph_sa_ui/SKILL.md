...
description: |
  ...
  define layout patterns, graph_sa_ui, UI architecture, components, navigation map.
---

# /graph_sa_ui --- UI Architecture (Graph)
...
- **`graph_core/SKILL.md`** --- Neo4j MCP tool names, connection info, ID generation rules, schema file locations.
...
| Form | FORM-{Name} | FORM-OrderCreate | Name-based (created by graph_sa_uc) |
| FormField | {FORM}-F{NN} | FORM-OrderCreate-F01 | Per-form (created by graph_sa_uc) |
...
3. If no match, propose creating a new DomainAttribute (suggest running `/graph_sa_domain MODIFY`).
...
Next:
  - If orphans remain: `/graph_sa_domain MODIFY {entity}` to add missing attributes
  - If all mapped: `/graph_sa_ui components` to identify shared components
```
...
Next: `/graph_sa_ui navigation` to define navigation structure.
```
...
Next: `/graph_sa_ui verify` to re-check form-domain mapping after navigation changes.
```
...
| /graph_sa_ui    |--->| /graph_sa_ui    |--->| /graph_sa_ui    |
...
Next:
  - `/graph_sa_validate` to run full cross-layer validation
  - `/graph_tl_plan` to begin technical planning
```
...
> No Form nodes found in graph. Forms are created during UC detailing. Run `/graph_sa_uc detail UC-{NNN}` first to create Forms and FormFields, then re-run `/graph_sa_ui verify`.

### No Modules in graph

If `navigation` finds zero Module nodes:

> No Module nodes found in graph. Modules are created during architecture design. Run `/graph_sa_architect` first, then re-run `/graph_sa_ui navigation`.

### Missing domain model

If `verify` finds data fields but no DomainAttributes exist at all:

> WARNING: No DomainAttributes found in graph. Form-domain mapping is impossible without a domain model. Run `/graph_sa_domain` first to create the domain model, then re-run `/graph_sa_ui verify`.
...
# Shared references:
- graph_core/SKILL.md         # ID rules, Neo4j connection, schema locations
```
...
| MAPS_TO | FormField | DomainAttribute | --- (fix only, normally created by graph_sa_uc) |
