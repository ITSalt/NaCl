...
description: |
  ...
  setup CI/CD, database migrations, or the user says "/tl-dev TECH###".
  Note: For UC backend tasks use /tl-dev-be, for frontend use /tl-dev-fe.
---
...
You work from self-sufficient task files created by `tl-plan`.
...
Run: /tl-review TECH### first to generate review feedback.
```
...
If `.tl/tasks/TECH-###/task.md` does not exist, report the error and suggest running `/tl-plan` first.
...
Run: /tl-review TECH### to start review
...
- `/tl-review TECH###` -- Start code review
