...
Write `.tl/tasks/UC###/sync-report.md` using `tl-core/templates/sync-report-template.md`.
...
| Verdict | Next step |
|---------|-----------|
| `PASS` | Proceed to `/tl-qa UC###` or `/tl-docs UC###` |
| `PASS_WITH_WARNINGS` | Proceed with warnings included in QA/review checklist |
| `FAIL` | Return to `/tl-dev-be UC### --continue` or `/tl-dev-fe UC### --continue` |

---

## Output: sync-report.md

Create `.tl/tasks/UC###/sync-report.md` using `tl-core/templates/sync-report-template.md`.
...
The developer runs `/tl-dev-be UC### --continue` or `/tl-dev-fe UC### --continue`, then re-runs `/tl-sync UC###`.
...
Report: .tl/tasks/UC###/sync-report.md
Next: /tl-qa UC### (warnings included in checklist)
```
...
Report: .tl/tasks/UC###/sync-report.md
Fix: /tl-dev-be UC### --continue  |  /tl-dev-fe UC### --continue
Then: /tl-sync UC###
```
...
Run: /tl-plan to create the task first.
```
...
Run: /tl-plan UC### to generate the API contract.
```
...
Run: /tl-dev-be UC### and/or /tl-dev-fe UC### first.
```
...
| API contract format | `tl-core/references/api-contract-rules.md` |
| Stub tracking rules | `tl-core/references/stub-tracking-rules.md` |

## Templates

- `tl-core/templates/sync-report-template.md` -- Sync report output format

## Examples

- `tl-core/examples/sync-report-example.md` -- Complete sync report example
...
**PASS / PASS_WITH_WARNINGS:** `/tl-qa UC###` (E2E testing) or `/tl-docs UC###` (documentation)
**FAIL:** `/tl-dev-be UC### --continue` or `/tl-dev-fe UC### --continue`, then re-run `/tl-sync UC###`
