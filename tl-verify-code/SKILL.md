...
description: |
  ...
  or the user says "/tl-verify-code".
---
...
## Key Difference from /tl-review

- `/tl-review`: checks code QUALITY (style, patterns, security, TDD compliance)
- `/tl-verify-code`: checks code CORRECTNESS (does the data flow work end-to-end?)

## Invocation

```
/tl-verify-code UC028               # verify specific UC implementation
/tl-verify-code --task ELE-644      # verify by task code (if YouGile)
/tl-verify-code --files src/routes/analytics.ts  # verify specific files
```
...
- Result structure: English (consumed by `/tl-verify` orchestrator)
- Findings detail: English (technical descriptions)
- User-facing summary: user's language

## References

- `tl-core/references/review-checklist.md` — for additional quality checks
- `tl-core/references/sa-doc-update-matrix.md` — for understanding doc impact
