...
description: |
  ...
  or the user says "/tl-fix" followed by a problem description.
---
...
```
/tl-fix "VK auth doesn't redirect back after login"
/tl-fix "500 error when completing interview"
/tl-fix "Typing indicator stuck when returning to dialog"
/tl-fix "Migration fails on prod: import.meta.url not supported"
/tl-fix "generation.test.ts — 8 failing tests"
```

Optional flags:
```
/tl-fix --dry-run "description"    # analysis only, no changes
/tl-fix --l1 "description"        # force L1 (skip docs)
/tl-fix --auto-ship "description" # after fix, automatically run /tl-ship
```
...
Use reference: `tl-core/references/fix-classification-rules.md` (if available; otherwise use the table above).
...
Use reference: `tl-core/references/sa-doc-update-matrix.md`
...
- Major domain model change → invoke `/sa-domain --mode=MODIFY` via Skill tool
- UC flow rewrite → invoke `/sa-uc --mode=update` via Skill tool
...
**Do NOT create full UCs from scratch** — that's the job of /sa-uc.
...
4. **Update .tl/changelog:**
   ```markdown
   ### [YYYY-MM-DD] tl-fix: [brief description]
   - **Level:** L0/L1/L2/L3
...
Next step — ship this fix:
  /tl-ship "fix: [short description from commit message]"

Recommendations:
  [if systemic issues found — suggest
   /tl-diagnose or /tl-reconcile]
═══════════════════════════════════════════
```
...
1. Automatically invoke `/tl-ship` with the fix description as commit message
...
3. In Report: recommend "/sa-uc for full specification of this area"
...
- `tl-core/references/fix-classification-rules.md` — L0/L1/L2/L3 classification rules
- `tl-core/references/sa-doc-update-matrix.md` — "code change → doc → skill" matrix
- `tl-core/references/tdd-workflow.md` — TDD cycle for Step 6
- `tl-core/references/review-checklist.md` — self-review checklist
- `tl-core/references/stub-tracking-rules.md` — stub markers
