...
description: |
  ...
  project health check, diagnose issues, or the user says "/tl-diagnose".
---
...
```
/tl-diagnose                              # full analysis (default: 7 days)
/tl-diagnose --since=5d                   # specific time period
/tl-diagnose --since=2026-03-21           # from specific date
/tl-diagnose --focus=interview            # focus on specific area
/tl-diagnose "nothing works after deploy" # with problem description
```
...
[IMPORTANT: for each discrepancy, include the doc file's last modified date
 so tl-reconcile can detect already-fixed items]
...
### If Health Score >= 60:
- Targeted fixes via /tl-fix:
  1. /tl-fix "[problem description 1]"
  2. /tl-fix "[problem description 2]"

### If Health Score 40-59:
- /tl-reconcile --scope=[top cluster]
- Then targeted /tl-fix for the rest

### If Health Score < 40:
- /tl-reconcile (full)
- After reconcile: /tl-stubs --final
- Then /tl-qa for critical UCs
```
...
- `/tl-reconcile` — as input for Phase 1
